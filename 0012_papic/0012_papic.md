...
> **✅ P3 SHIPPED 2026-06-12 (PR #1282, no migration — composition only).** The day-of console at **`/dashboard/[eventId]/live`**, couple OR coordinator (the authority every wall RPC already checks): status counts (on-wall / hidden / screens connected) · **lifecycle mode control** Auto/Teaser/Live/Recap writing `events.live_mode_override` (Auto = NULL, server derives from event date; covers the two real day-of moments — open the wall EARLY, freeze to RECAP at program end) · **FaceBlock posture strip** (blur-ready coverage of recent tiles — "why is the wall thin?" answerable at a glance) · the P1 screen-code + kill-switch controls REUSED at 60 tiles · the Kwento approval queue REUSED inline (the "Kwento lower-thirds" half of this build-order row shipped with Kwento P2's wall rendering). "Live Wall" nav entry in the Share group; events without LIVE_WALL get a quiet add-on doorway (no price copy). `setWallMode` checks couple/coordinator at app level and writes via the admin client (the shipped `couple_can_update_event` RLS policy is couple-keyed). **Salamisim remaining: P4** (SW cache + recap freeze `wall_freeze_recap`) **· P2b** (per-guest selective blur — needs the enroller + a validated embedding model).
>
