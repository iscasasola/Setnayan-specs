...
- **No CUSTOMER-side convenience fee** — couples are never surcharged. ⚠ **But the vendor side is no longer 0%: the BOOKING FEE (5% first ₱100k · 1% above · floor ₱50 · no cap, sourced clients only, first 5 free) was owner-locked 2026-07-25.** See the SKU section above; the rate lives in `lib/booking-fee.ts`, never in a doc. ⚠ **"Setnayan does not hold money" is ALSO wrong for in-app vendor bookings** — `lib/payouts.ts` (locked 2026-05-16) receives the couple's payment and disburses to the vendor: verified ⇒ T+1 full; unverified ⇒ 20/60/20 with a dispute window; net of BIR 0.5% withholding, Setnayan absorbing the ₱15–25 transfer fee. The admin "release" button **records a transfer you made by hand** (method + reference) — it does not move money. A separate automated **Setnayan Pay gateway** (per-rail 1.5%/2.0%/2.5% in `setnayan_pay_methods`) is **dormant** — every row is `is_active=FALSE`, not charged in V1.
