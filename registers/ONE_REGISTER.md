...
| LR-7 | sweep | ~~Nothing proves a signup email is real~~ **— that half is WRONG. The typo half is CONFIRMED.** | 🟠 **HALF RETRACTED 2026-09-17.** Email confirmation IS on and working: `auth.users` holds 13 rows — **8 non-anonymous, and all 8 have `email_confirmed_at`**; the other 5 are the anonymous accounts of LR-12. Nobody has an unconfirmed address. ⇒ **THE SECOND HALF HOLDS AND IS THE REAL ROW:** there is **no email-change path for anyone, user or admin**. The only `updateUserById` reference in the tree is inside a comment in `lib/erasure/coverage-guardrail.test.ts`. **A typo at signup is permanent**, and with LR-6 (a reset only completes in the browser that asked) it is unrecoverable. 🔑 A compound row where one half measured true and the other measured false — **split rows like this before building, or the true half gets discarded with the false one.** | no | a-session | `select count(*) filter (where is_anonymous), count(email_confirmed_at), count(*) from auth.users;` + `git grep -n updateUserById -- apps/web` |
