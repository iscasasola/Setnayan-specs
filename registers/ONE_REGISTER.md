...
| DAY-4 | **EH-MONEY** ⇄ BF **S6** | The Event Hub controller shows what this event already owns, as a filling meter | NOT BUILT — ⚖ **NEEDS THE OWNER (S31, 2026-09-18).** Design found: `EVENT_HUB_CONTROLLER_DESIGN_2026-09-02.md` §3.3 S6 + `prototypes/service_control_center_pattern_2026-08-28.html` ("credits or plan … with the ladder beside it · only for services that have one"). **It names the slot and never names the quantity.** The Hub already shows owned/not-owned per service (S4 rows on `launch/page.tsx`), so the only new thing is the METER, and two readings fit: (a) the event's Papic credit balance against the credit ladder; (b) add-ons owned out of add-ons offered for this event type. Both shape how spending is presented, and the design's own §0a left S6 unbuilt under §5.1 rule 5 ("don't guess"). **Owner question: which number fills the meter, and against what total?** | ⚖ which quantity | S–M · Opus | — |
...
| DAY-24 | BF Samahan | Photos can be sent in the group chat (Usapan); a Memories tab of past group moments | NOT STARTED — ⚖ **NEEDS THE OWNER (S31, 2026-09-18); this row's "no" gate was wrong.** `WHATS_NEXT_Samahan_2026-08-24.md` §2e records "no photos in Usapan" as **deliberate** (another moderation surface), and §3 decision 1 ("what a samahan KEEPS, and whether keeping is what we sell") is still open. A Memories tab IS that shelf, and a chat photo forces the same retention choice. The one ruling on record is the owner's 2026-08-24 *"keep these videos for 24 hours"* for stories, with keep-forever routed to Papic on a group event. **Owner questions: (1) may Usapan carry photos at all, given the moderation load? (2) if yes, do they expire at 24h like stories or persist, and is a persistent Memories shelf free or paid?** Build note: stories already carry member uploads (`samahan-stories-24h`), so a 24h chat photo can reuse that path with no second upload pipe. | ⚖ keep + moderation | S · Sonnet ·rule | — |
