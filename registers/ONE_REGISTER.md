...
| DAY-14 | **LS-DAYCOPY** | The controller stops offering "Add another day" now that Live Studio is one permanent unlock per event | 🔴 **CONFIRMED OPEN — re-measured 2026-09-16, and it is charging people. `apps/web/app/panood/control/[eventId]/page.tsx` still says, in the `entitled` branch — i.e. to a couple who ALREADY BOUGHT Live Studio — "Your broadcast day has ended, so live switching is paused. Add another day (₱X) to cut between cameras on air again."** LS6 made Live Studio a **permanent one-time unlock per event** and deleted the broadcast-day model (`lib/live-studio-window.ts` is gone). ⚠ **AND A TEST IS HOLDING THE FALSE COPY IN PLACE:** `lib/live-studio-wave8-layout.test.ts` asserts the "Add another day" button is never clamped, so removing the copy turns it red — change the test in the same PR and say so. The button lives in `_components/broadcast-window-strip.tsx`. 🔑 Read the price from `platform_retail_catalog_v2`, never from code or a comment. **Assigned to the DAY block owner 2026-09-16.** | no | S · Sonnet | Live Studio is a **₱2,500 one-time unlock**, not per-day |
...
| LR-8 | sweep + prod | **The Verified badge means nothing — BOTH shops wear it and NO verification record exists for either** | 🔴 OPEN 2026-09-16 · **RE-MEASURED TWICE, and WIDENED on the second pass.** `select count(*) from vendor_profiles where public_visibility = 'verified'` = **2**; `select count(*) from vendor_verifications` = **0**. Both shops — `Saysay Live Band & Hosting (FIXTURE)` AND `SetnaProd` — are publicly Verified with `is_demo = false`, so every demo filter passes them through to `/`, `/explore` and the sitemap. ⚠ **THE FIRST READING OF THIS ROW WAS "rename the fixture", AND THAT WAS A DATA FIX FOR A MECHANISM FAULT.** `SetnaProd` is the proof: it is not a fixture, it carries the same unbacked badge, and renaming the other one would have left the badge meaning nothing while looking resolved. 🔑 **A row can be true and still point at the wrong repair — the second instance is what tells you which.** | ⚖ owner: what the badge must require | a-session (the invariant) + owner (the policy) | **Assert the INVARIANT, not the names:** `select count(*) from vendor_profiles where public_visibility = 'verified'` must NEVER exceed `select count(*) from vendor_verifications`. |
...
| LR-13 | sweep | A guest's photo withdrawal hides five pages and **leaves the file public forever** | 🔴 OPEN 2026-09-16 · unsigned public media host. ⚠ **IT CONTRADICTS THE SAME-DAY RULING IN A PRECISE AND UNCOMFORTABLE WAY:** #5537 (merged and served 2026-09-16) makes her withdrawal **final and un-reversible by the couple** — so the product now keeps the half that is hard to build and breaks the half she actually cares about. **The couple cannot put it back, and the photograph never left.** 🔑 Whoever builds this asserts the OBJECT DELETE, not the row hide — a hide that five pages honour is exactly what made this look finished. | no | a-session | assert the takedown path calls the object delete; a row whose `wall_hidden_by_guest_id` is set must have no reachable object |
| LR-5 | sweep | Supabase Auth has no custom SMTP — capped at **2 emails/hour platform-wide** — so a locked-out stranger stays locked out | 🔴 OPEN 2026-09-16 · Resend is configured and paid for; Auth is simply not pointed at it | ⚖ owner-config ~20 min | hours · owner | Dashboard → Project Settings → Auth → SMTP; verify with two resets in one hour from an external address |
| LR-9 | sweep | The front page promises suppliers **"No commission on your bookings, ever"** while a 5% booking fee ships | 🔴 OPEN 2026-09-16 · `/vendors` contradicts `/` on the same crawl. ⚠ Interacts with the open booking-fee free-tier question — **do not write the sentence until that is ruled** | ⚖ owner: one sanctioned sentence | a-session | assert the PROPERTY, never the phrasing: no public page may promise zero fee while `booking_fee_rate_pct > 0` |
...
| LR-4 | sweep | `samahan-story-sweep` is **twelve days past a ONE-HOUR gap** — the claim path is broken for it | 🔴 OPEN 2026-09-16 · `gapMs = 60*60*1000`, last claim 2026-09-04. Not low traffic | no | a-session | `grep -n -A3 "samahan-story-sweep" apps/web/lib/periodic-job-registry.ts`, then `select job_key, last_run_at from cron_job_runs where job_key='samahan-story-sweep';` |
| LR-6 | sweep | A password reset **only completes in the browser it was requested from** | 🔴 OPEN 2026-09-16 · PKCE verifier cookie. Ask on the laptop, open on the phone, get a paragraph of SDK docs | no | a-session | `ls apps/web/app/auth/confirm/route.ts` → expect ENOENT; the fix is the server-side `token_hash` pattern |
| LR-7 | sweep | Nothing proves a signup email is real, and **nothing can correct a typo — not even an admin** | 🔴 OPEN 2026-09-16 · one typo = a permanently unreachable account | no | a-session | `grep -rn "updateUser({ email" apps/web/lib` → expect no writer beside changePassword |
...
| LR-14 | sweep | "Replace this guest's QR" leaves an already-opened browser with **full access for up to 60 days** | 🔴 OPEN 2026-09-16 · the closing check exists, behind `guestSessionTokenCheckEnabled()` | no | hours | `grep -n "guestSessionTokenCheckEnabled" apps/web/lib/guest-session.ts` — the fix is to **DELETE the flag**, not remember it |
| LR-15 | sweep | `receipts.issued_to_tin` has readers and **no writer**; the receipt never itemises and promises a BIR document nothing produces | 🔴 OPEN 2026-09-16 · ⚠ **narrowed by the refuter**: the statutory half is the BIR registration + an ATP invoice series under the EOPT Act, already in `DECISION_LOG` — **only the TIN field and itemisation are ours** | ⚖ owner: registration | a-session | `grep -rn "issued_to_tin" apps/web --include=*.ts --include=*.tsx \| grep -v test` |
| LR-16 | sweep | Switching a payment rail off **does not stop 5 of 7 payment screens taking money on it** | 🔴 OPEN 2026-09-16 · matters the moment a GCash or BDO account is capped or frozen | no | a-session | assert every payment surface reads `gcash_enabled`/`bdo_enabled` before rendering a QR |
