...
- ✅ **ANSWERED 2026-08-27 — "yes. they can declare it."** Do not re-ask. ⚠ And the question's own
  ...
  what shipped. ✅ **ALSO ANSWERED 2026-08-27 — "yes they keep their record."** The refusal marks the
  claim instead of erasing it; nothing about the couple's own record is deleted. Do not re-ask.
...
| 8 | Somebody says they paid you | ✅ **BUILT · MERGED · SERVED 2026-08-27** — PR [#4923](https://github.com/iscasasola/setnayan-platform/pull/4923); prod `/api/health` self-reports the merge commit `97cfe10`. — owner: *"yes. they can declare it."* Both conditions shipped: the row carries the receipt (*"See what they sent"* — `proofUrl` had been fetched into that card since it was written and **never rendered once**, so the answer was made blind) and a second button behind a fold. 🔑 **THIS ROW'S PREMISE WAS WRONG: the "no" was ALREADY BUILT** — `vendorRejectDeposit` → the `reject_vendor_deposit` RPC, with an ownership gate, a single-winner UPDATE and a reason that reaches the couple, wired to a control on the supplier's customer card. **The DESK had no no**; the mechanism did. A 334-line duplicate migration was written and deleted on finding it. ✅ **AND THE WIPE IS FIXED — owner ruled 2026-08-27: _"yes they keep their record."_** PR [#4927](https://github.com/iscasasola/setnayan-platform/pull/4927), **MERGED + SERVED** (prod `/api/health` self-reports `73cee68`), and migration `20271175634994` is **verified applied in prod BY THE OBJECT**: 3 new columns · the one-way CHECK · `reject_vendor_deposit` contains **no** claim-erasure and **no** ledger DELETE and does stamp the refusal · `acknowledge_vendor_deposit` clears it · the guard covers the refusal AND still covers the acknowledgement. The refusal is a MARK now (`deposit_declined_at` + the supplier's words + who): the couple keeps their amount, receipt, method and ledger row, their card quotes the supplier's words and offers **Send it again**, and re-sending clears the refusal so the question returns to the desk. 🚨 **The downstream effect that would have bitten: `supplierWasPaid` counts a recorded deposit and a ledger row as PAID, and a paid unreleased supplier BLOCKS the couple deleting their own celebration** — keeping those two facts would have made a celebration permanently undeletable behind a supplier who says they were never paid. A refusal now suppresses exactly those two signals, which is byte-equivalent to what the old erasure produced |
