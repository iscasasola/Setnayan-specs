...
- **Pre-venue → realign lifecycle (owner-clarified 2026-06-04):** location filtering is **always on**, even before any reception venue exists. **Phase 1 — onboarding (no locked venue):** the **onboarding area pick(s)** ARE the working anchor — their city centroid(s) drive the serves-area filter + Nearest sort. The onboarding pick is *held* as the pre-venue stand-in (not thrown away the instant onboarding ends), so vendors are filtered by location from day one. **Phase 2 — reception venue chosen/locked:** the anchor **realigns** to the venue's coords (the precedence above takes over; the onboarding area stops mattering). **Phase 3 — shortlist:** when ≥1 vendor is shortlisted and **2 candidate areas/venues** are in play, each shortlisted vendor is **labeled with which reception venue/area it is nearest to** (the min-distance candidate) — so "near" reads concretely per option ("nearest to your Tagaytay venue"). Single-area weddings skip the label.
...
- **The reception venue is the stored anchor.** `events` persists the chosen **reception venue identity + `venue_latitude`/`venue_longitude`**. Onboarding's **"pick up to 2 areas"** scopes the *reception-venue search* AND serves as the **Phase-1 working anchor** while no venue is locked (its city centroid drives the location filter — §2a lifecycle). It is **not a permanent hard-filter**: once a reception venue is chosen, its coords take over (**realign**) and the area stops mattering. So the area is the pre-venue *stand-in* for the venue coords, not a stored region gate.
...
**Entry:** a **"Personalize my matches"** button on the dashboard event-home (Home), persistent. (Distinct from the Setnayan AI wizard — this is the *preference store*, not the task guide.)
...
| **`fetchWizardVendorRecommendations`** (the canonical ranked query) | Gains the preference-partition + capacity/date/area hard-filter params + the Expand (return-excluded) flag. This is the single place the sort lives — overlay, marketplace, and Setnayan AI vendor cards all call it. |
| **Setnayan AI wizard vendor-pick cards** | Inherit the same preference sort automatically (they already call the canonical query). |
