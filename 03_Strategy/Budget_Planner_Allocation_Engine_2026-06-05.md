...
**Forward-flag (✅ RESOLVED 2026-06-09):** the finer **"linked-services-on-card"** concept ([[project_setnayan_booking_ruleset]]) now HAS a schema — table `vendor_service_links` + the **solo-vs-linked marker `vendor_services.is_linked_only`** (migration `20261014000000`, PR #1187). **TODO (not yet wired):** the median filter must now also `WHERE is_linked_only = FALSE` so linked-only rows (priced as part of an anchor, not standalone) don't depress a leaf's solo-price median — "exclude bundles" alone no longer fully covers the rule now that linked-only `vendor_services` rows can exist.
