...
- **Booth/stage placements become first-class floor objects** (the 0008 catalog already free-places a stage): couple or coordinator drops "Photo Booth — [vendor name]" on the plan; that placement pin is what the booth vendor's Brief deep-links to. **→ BUILT 2026-06-13 as MULTI-AREA BLUEPRINTS (PR [#1309](https://github.com/iscasasola/setnayan-platform/pull/1309), owner-approved extension):** `event_floor_areas` (cocktail garden / foyer / ceremony spaces beyond the reception room, each optionally tied to a schedule block for its live window — "cocktails 5–7 PM while the reception flips") + `event_floor_objects` (drag-placed booth/station pins, optionally linked to the booked vendor). Couple surface `/seating/areas`; vendor viewer renders every area with the vendor's own pin highlighted ("Your spot").
...
|---|---|---|
| **1. Vendor Event Brief — ✅ BUILT 2026-06-12 (PR [#1296](https://github.com/iscasasola/setnayan-platform/pull/1296))** | SECURITY DEFINER RPC `get_vendor_event_brief` (migration `20261128000000`, applied to prod) + `/vendor-dashboard/clients/[eventId]` brief page. Aggregates only; dietary gated to food-relevant categories + coordinator; full timeline per D2. Deferred within phase: crew-meal total (no booking→service linkage yet) · per-vendor field raise/lower (needs Phase 2 grants). | Biggest value-per-effort; ships alone safely |
| **2. Coordinator delegate — ✅ BUILT 2026-06-13 (PR [#1300](https://github.com/iscasasola/setnayan-platform/pull/1300))** | Migration `20261129000000` (applied to prod): `moderator_area_level()` per-area RLS grants + publish guard trigger (`publish_requires_couple`) + delegate-audit triggers into the adopted 0016 `event_action_log`. Layout admits accepted moderators; accept flow adds `event_members` coordinator row; /hosts gains "Promote your coordinator" + grant chips + budget toggle (D1) + remove + activity stream. Deferred within phase: chat join-all (→ Phases 3/5) · delegate-view UI polish. | Foundation table already shipped; unblocks every "coordinator edits X" promise. **DB-level guards first** per the conflict-architecture lock — delegate writes are the first true 2-writer surface on couple data |
| **3. Shared timeline + Suggest flow — ✅ BUILT 2026-06-13 (PR [#1303](https://github.com/iscasasola/setnayan-platform/pull/1303))** | Migration `20261130000000` (applied to prod): booked-vendor live read on `event_schedule_blocks` (full timeline, D2) + `event_schedule_suggestions` propose/resolve substrate. Vendor Brief gains per-block "Request a change" + new-entry suggest + own-request statuses + per-vendor `calendar.ics`; couple Schedule gains the "Vendor requests" Accept/Decline queue (accepted `new` lands as private draft). Deferred in-phase: own-slot pinning (→ Phase 4 linkage) · vendor withdraw · 0028 notification fan-out. | Needs phase 2's approval actor (coordinator) to be useful |
| **4. Live viewers — ✅ BUILT 2026-06-13 (PR [#1305](https://github.com/iscasasola/setnayan-platform/pull/1305)) · booth pins deferred** | `get_vendor_seat_plan` RPC (migration `20261201000000`, applied): publication-gated, floor-category-gated, counts-only published floor plan + per-table meal counts for food categories; viewer at `/vendor-dashboard/clients/[eventId]/seat-plan`. **Booth placement pins deferred** to a dedicated 0008-editor slice (new floor-object type in the editor). | Reuses print-pack rendering; pins extend the 0008 catalog |
| **5. Invitation campaign delegation — ⚙ RESOLVED-AS-BUILT 2026-06-13 (no new code; D4 gate parked)** | **As-built finding:** the product has NO bulk invitation-email campaign — invitations are per-guest QR/claim links the couple shares via their own channels (0002/#1220); the only send is the claim-approval email. The delegation half is therefore **already live via Phase 2 RLS**: an invitations/guest-list-edit delegate can reissue guest tokens, manage the list, and chase RSVPs (the invitation surface's actions are pure-RLS). The **D4 hard couple-confirm + guided-UX spec (§ 5) is PARKED until an invitation campaign-send mechanism exists** (0028 territory — save_the_date_sent / bulk sends); when that ships, § 5's gate spec applies verbatim. | Smallest slice; rides phase 2 grants |
