...
- **Layout: results on top, chosen songs below, search bar pinned at the BOTTOM** (mirrors the location screen, screen 7). Refined 2026-06-05 into **two SEPARATE bounded scroll regions** so results never overlap the chosen songs, and the **search bar can never be pushed off-screen**: (1) the whole prefs chain (`#screen-prefs` → `#prefhost` → `.prefstep:has(.songbank)` → `.tapzone`) is made shrinkable (`flex:1 1 auto; min-height:0`, same pattern as `#screen-pax`/`#screen-budget`) so the bank's regions scroll internally instead of the page; (2) **results** rise just above the search bar (`margin-top:auto`) and scroll internally; (3) **chosen songs** get their own region **capped at half the screen** (`max-height:50dvh`) and scroll internally; (4) both regions may shrink so the pinned search always stays on screen. Browser-verified mobile (375×812) + desktop (880 frame): worst case (25 results + 12 chosen) → page doesn't scroll, search fully visible, 8px gap between regions, chosen ≤ 50%.
