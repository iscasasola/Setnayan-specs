...
4. **LINKED SERVICES** — other tiles bundled, auto-tagged **"✓ included with {vendor}"**; **card shows "comes with [X][Y][Z]."** _(✅ **BUILT 2026-06-09** — table `vendor_service_links` (anchor service → `linked_canonical_service` + `linked_label`) + `vendor_services.is_linked_only` marker; couple card renders "✓ comes with X · Y · Z"; vendor authors via the `/vendor-dashboard/services` "Comes with" picker. Distinct from `vendor_packages` bundles. Migration `20261014000000`, PR #1187.)_
...
- **Per-category schedule pools (owner 2026-06-12):** the schedulable resource is the **(org, leaf-category)** pool — not the shop, not the listing. All services filed under the same leaf category draw from **one shared schedule** (five photography packages = one photography team; booking any of them consumes the same slot). A **new category = a new, independent schedule** (different materials/crew — a caterer-florist can serve two couples on the same date). Capacity (`daily_booking_capacity`, Enterprise time slots) attaches to the **pool**, superseding today's per-service `vendor_services.daily_capacity` grain. The tier ladder bounds calendar count automatically (Free 1 · Pro 3 · Enterprise unlimited categories). Vendor calendar UI = one calendar, one swimlane/tab per category.
- **Opt-in pool MERGE:** a vendor whose categories share one crew (the classic photo+video studio) can merge those categories into **one shared pool** at setup ("same team serves both"); merged pools behave as a single schedule. Per-category stays the default. *(Owner 2026-06-12: "whatever is proper to our plan" — merge adopted as the escape hatch so shared-crew vendors don't hand-mirror blocks.)*
- **Bundles lock every pool they span (owner 2026-06-12):** a cross-category bundle / linked-service booking consumes a slot in **each** category pool it draws from, via **one all-or-nothing multi-pool atomic acquire** — partial acquisition must be impossible (a half-booked bundle double-books one side). Single-category bundles consume their one pool.
- **Block scoping:** an imported **external client** (the vendor's off-app booking — optional name/contact/notes attached to a block; **not** an app client: no thread, no funnel stats, no review eligibility) consumes **one category pool**. A vacation/closure block is **org-wide** across all pools. Both render to couples as plain "unavailable" (privacy rule above). Displacement-clears fire **within the affected pool only** — inquiries on the vendor's other categories for that date are untouched.
...
- **Ignore → 30-day auto-expiry** — the inquiry is status-flipped to `expired` (event-driven, **no cron**), and it counts toward the vendor's responsiveness signal.
...
**Lifecycle is status-flipped, NEVER hard-deleted (2026-06-12).** Canonical inquiry states: `active → on_hold` (a competitor's lock request opened the 48h exclusive hold) `→ displaced` (slot filled — **revivable**) **|** `converted` · `declined` · `withdrawn` · `expired` (terminal). Everywhere this doc says "cleared / auto-deleted," implement as a **status flip** — UX-identical (hidden from all active lists) but the row persists for revive, the responsiveness-rate denominator, the behavioral-data edge, and dispute audit. *(Amends the 2026-06-04 "auto-deleted" wording; also retires the shipped `deleteVendor` hard-delete that orphans chat threads.)*

**Revive on fall-through (2026-06-12).** A fall-through **before Payment-Verified displaces nobody** (removals execute only on Payment-Verified — §3). If a BOOKED (`deposit_paid`) lock later collapses (cancellation / force majeure), the `displaced` inquiries for that pool+date are **re-validated** (event date still future · category still unfilled on the couple's side · candidate date still valid) and revived as a **"this date reopened — still interested?" confirmation prompt** — never silently re-queued (couples may have moved on; vendors shouldn't see ghost inquiries). Confirmation resets the vendor's SLA clock so the revival can't generate an unfair strike.

**Conflict-notification tiering (2026-06-12).** Competing *inquiries* consume nothing (white = unlimited) → **no per-inquiry push** (noisy, anxiety-inducing, gameable). Instead: **(a)** an ambient demand signal on the card/thread — "N other couples are inquiring for this date" (eventually-consistent, **min-N floor** so nobody is identifiable); **(b)** push only on material state changes — the 48h exclusive hold opening ("this date is on hold — it may close") and Payment-Verified (the one broadcast displacement notice above).
...
**Confirmed (2026-06-12):** **per-category schedule pools** — one shared schedule per (org, leaf-category); same category = one pool, new category = new independent schedule · **opt-in pool merge** for shared-crew categories · **bundles lock every pool they span** (multi-pool all-or-nothing atomic acquire) · **inquiry lifecycle = status-flipped, never hard-deleted** (`active / on_hold / displaced / converted / declined / withdrawn / expired`) · **revive-with-confirmation** on post-`deposit_paid` fall-through (pre-Payment-Verified fall-through displaces nobody) · **conflict-notification tiering** (ambient min-N demand count + push only on 48h-hold and Payment-Verified) · **external imported clients** = category-scoped capacity-consuming blocks, not app clients.
...
3. **Slots + org-level shared schedule + white/locked view** — ✅ **BUILT 2026-06-12** (3-PR program: schema `20261126000000` — pools/mappings/reservations/blocks + `acquire_schedule_pools` multi-pool all-or-nothing + `release_schedule_pools` + lifecycle enum values · wiring — `updateVendorStatus` acquires on white→BOOKED + releases on downgrade, `deleteVendor` booked-row guard · surfaces — `/vendor-dashboard/calendar` per-pool tabs/grid/blocks/capacity/merge + `/vendor-dashboard/clients` booked/in-conversation/outside + `import_external_client` 1-token atomic import `20261127000000`). **Still open in this phase:** pool-scoped inquiry displacement + one-broadcast notice (needs thread→service linkage) · revive-with-confirmation flow · event_vendors hard-delete→archive sweep · true multi-listing org table (pools key on `vendor_profile`, which IS the org today — user_id UNIQUE).
