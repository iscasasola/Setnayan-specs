...
**Action:** review + fold into the canonical [Schedule_Matrix_and_Date_Finder_2026-06-02.md](Schedule_Matrix_and_Date_Finder_2026-06-02.md) via Cowork (proposed new sections §12–§17). **§F cultural-depth decision RESOLVED 2026-06-04 (soft-caution).** **§L demand / hot-date heat layer added 2026-06-09 (owner: include internal inquiry statistics in the hot score).** Still open: the crewed-services day-capacity question (§B2) + the owner-to-set values (lead-time curves, mode weights, traffic/typhoon tables, **§L demand weights + min-N + %-fill cutoff**).
...
## §L. Demand & the "hot-date" heat layer (the parallel readout · added 2026-06-09)

**What it is.** Distinct from *fit* ("is this day right for **you**?"), **demand** answers "how many **others** want this day?" — surfaced on the *same* calendar as a **heat color (transparent → red) + a 1–4 flame count** with a tap-nugget. Prototype: [`Hot_Date_Heat_Calendar_Prototype_2026-06-09.html`](../06_Prototypes/Hot_Date_Heat_Calendar_Prototype_2026-06-09.html). It is **one sub-score, not a new engine** — it feeds the **D4** surge framing and rides alongside the §C modes (a hot date can be the *top pick* **or** be *down-ranked* under Least-stressful for crowding + premium pricing). Demand is **information, never a verdict** — same posture as the §F soft-cautions.

> **Definition — when is a date "hot"?** A date is labeled **hot when its demand sub-score reaches the "In-demand" tier (≥ tier 3 of 4).** Two sources feed that score, blended by confidence:

### L.1 — Predicted demand (deterministic · cold-start-safe)
Stacks calendar signals from reference tables — works day-one with **zero** marketplace volume:
- peak-month weight (Dec/Feb/Jan/Nov high · Apr/May/Oct shoulder) · **Saturday** (+) / Sunday (+) · **repeating-number** date (12/12, 11/11…) · Valentine's / long-weekend · combo bonus.
- Tiers **Open · Quiet · Popular · In-demand · Hottest** (0–4). "Hot" = ≥ In-demand; "Hottest" = three signals stack (e.g. `12·12` + Sat + Dec).
- Weights + tier cutoffs are **owner-to-set** (§J.1), never hardcoded magic numbers.

This half is a **forecast** — *"history says this books out."*

### L.2 — Observed demand (data-dependent · from Setnayan's own app statistics) — *owner addition 2026-06-09*
The **measured** half — *how many couples are actually inquiring on / targeting a date inside our app.* First-party Supabase only (the **behavioral-data moat**). Inputs, all **compute-on-read · cron-free**:
- **Date interest** — # active couples whose `date_candidates` / `date_window` / locked `events.event_date` include the day.
- **Inquiry pressure** — # vendor inquiries (`chat_threads`) + shortlists (`event_vendors`) on events targeting the day, **weighted by funnel depth** (eyeing < inquired < downpayment < booked — a paid booking counts far more than a glance).
- **Vendor fill** — % of relevant / in-area vendors already blocked or booked that day (`vendor_calendar_blocks` + capacity).

**Demand-pressure escalation — RELATIVE TO SUPPLY (owner-chosen 2026-06-09 · the core mechanic).** The escalation is driven by the **demand-to-available-supply ratio**, *not* an absolute inquiry count — so it stays meaningful at **any** marketplace size (one rule fixes cold-start *and* scale). The headcount **raises the tier** and can lift a date **past its L.1 forecast** — a plain weekday becomes *In-demand* because many couples are converging on it — which is legitimate: **every interested couple competes for the same finite vendor pool that day, so the *ratio* is real scarcity.**

```
pressure(date) = contested(date) / available_supply(date)
   contested        = funnel-weighted interested couples for the date, in the couple's region (eyeing < inquired < booked)
   available_supply = relevant vendors NOT already blocked/booked that day, in-region,
                      taken at the BOTTLENECK category (the date is hard because, say, every photographer is gone)

pressure_boost (admin-set · illustrative):  <25% → +0 · 25–49% → +1 · 50–74% → +2 · 75–99% → +3 · ≥100% (more couples than free vendors) → cap at Hottest
tier = min(Hottest, predicted_tier + pressure_boost)
```

- **Why relative beats a flat count (the "every-10?" question, answered):** 10 inquiries against 100 vendors is nothing; 10 against 8 is a sell-out — only the *ratio* captures scarcity, and it **auto-scales** as supply grows (so we never re-tune a magic "10"). *(Retires the earlier absolute-count band proposal.)*
- **Floors (anti-noise + privacy):** two *separate* guards — a **privacy min-N on couples** (never *display* a count below N) and a **supply-confidence fade** that damps the boost when few vendors exist (1 free vendor + 1 inquiry ≠ Hottest). Cold-start / small-market behavior is detailed in **§L.3**.
- **Nugget reason** (above min-N): *"👀 14 couples are chasing the 9 vendors free that day (156% booked-pressure) — that's pushed it hotter."*

This half is **measured** — *"vendors are actually filling up."*

### L.3 — Cold-start & small-supply behavior (does relativity need a big marketplace? — no)
The observed layer is **scoped to region × bottleneck category, not the whole platform** — so it activates **scope-by-scope** as each niche fills, never all-or-nothing. That answers "only once we have lots of venues, or even while small?":

- **It does NOT need a large *global* marketplace — only enough supply *in that scope* to be trustworthy.** A **small but saturated** niche is *legitimately* hot: a town with 3 photographers, all booked/inquired for a date, is 100% contested and the model **correctly** reads it hot. Relativity is the point — small supply that's full = real scarcity.
- **Graceful confidence-fade, not a hard on/off (recommended over a binary floor).** The observed boost is **weighted by supply size** — `confidence = supply / (supply + k)` — so 1–2 vendors barely nudge the tier (noise damped) and the signal **sharpens smoothly** as vendors join. No magic cutoff; a thin market *fades in* instead of flipping. Below a tiny-supply minimum it contributes nothing and the L.1 forecast shows with an **"early signal"** confidence label (§E 3-state posture).
- **Two distinct floors — keep them apart:** (a) **privacy min-N on *couples*** — never *display* a count below N (RA 10173), a hard rule; (b) **statistical confidence on *supply*** — how much the ratio is *allowed to move* the tier, the graceful fade above.
- **Confidence-weighted blend:** predicted dominates while data is thin; observed takes over per scope as volume grows. Same red either way — only the *truth behind it* upgrades from "history says" to "vendors are actually filling."

**Today (founder-only marketplace): the observed layer is effectively off everywhere — the L.1 calendar forecast carries 100% of the heat.** Correct and honest: predicted demand needs zero data. The relative layer lights up region-by-region, category-by-category, as each fills — no flip-the-switch moment, no regression.

### L.4 — Privacy (RA 10173 + behavioral-data lock)
Inquiry stats are **top-protection-class first-party data.** Aggregate + de-identified only: the couple sees the **color**, never *who*. A raw count / "most-requested this month" social-proof line renders **only above min-N**. One couple's date is **never** used to target another. Carries parent §8 + this doc's §I.

### L.5 — Self-reinforcement caution
Surfacing "hot" **changes behavior** (herding toward — or fleeing from — a date) → biases future stats. Mitigation: the recommender's *scarcity / hidden-gem* logic keys off **vendor availability**, not couple-interest alone — couple-interest is honest social proof for *wanting* the date, but only vendor fill proves *scarcity*. Admin **kill-switch** + monitoring on the observed layer.

### L.6 — Three-actor surfaces
- **Customer** — heat color + flame count + (above min-N) a "most-requested" nudge; demand never *blocks* a choice.
- **Vendor** — high-inquiry dates cue marking **peak/premium** (§B2) and keeping availability open; *"N couples are seeking [category] around [date]"* (aggregate · min-N).
- **Admin (0023)** — the **inquiry-density analytics** surface (parent §7 + owner #10 trends): real heat by date / region / category, threshold tuning, the kill-switch, audit. Where owner point #8 (*evolves as data grows*) lands for dates.

### L.7 — Thresholds = admin-owned (§J.1 additions)
New reference-table entries, all **owner-to-set**: month weights · tier cutoffs · **min-N** · **%-vendor-fill → hot** cutoff · the predicted↔observed blend / confidence curve · per-layer kill-switch.
...
| + | Internal inquiry stats → a date's demand score | **§L.2 observed demand** (owner addition 2026-06-09) — measured inquiry/eyeing/fill volume, min-N-gated, blended with the §L.1 forecast |
