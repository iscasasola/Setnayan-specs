...
> **Part B** — the NPC Data Processing System Registration data sheet: the field-by-field information required to complete registration through the **NPC Registration System (NPCRS)**.
>
...
>
> **Never-invent rule:** every value not supplied verbatim is marked `[TO CONFIRM]`. Do not guess.
...
| **BIR registration (Form 2303 / TIN)** | `[TO CONFIRM]` |
...
| **Total employees / headcount** | `[TO CONFIRM]` |
| **Total number of data subjects** | `[TO CONFIRM]` |
| **Number of data subjects with sensitive personal information** | `[TO CONFIRM]` |
...
| **Effectivity date of designation** | `[TO CONFIRM]` |
...
5. **Handle data-subject requests** — receive, coordinate, and ensure timely response to data-subject requests to be informed, to access, to object, to rectify, to erase/block, to data portability, and to be notified of breaches (Policy § 9 and Appendix A; committed response window: **within 7 business days** for non-urgent requests, **immediate** for breach notifications).
...
This designation takes effect on **`[TO CONFIRM]`** and remains in force until revoked or superseded in writing.
...
| Signature | ________________________________ |
| Name | **Indalecio Sacdalan Casasola II** |
| Title | Data Protection Officer · `[TO CONFIRM position]` |
| Date | ________________________________ |
...
*This is the field-by-field information to be entered into the **NPC Registration System (NPCRS)**. Values are drawn from the master fact block; `[TO CONFIRM]` marks fields that must be settled before filing. The **actual online filing is performed by the DPO / owner** through the NPC Registration System — this sheet is the data to file, not the filing itself.*
...
|---|---|
| Processes **sensitive personal information of at least 1,000 individuals** | **Likely YES** — face vectors + government-ID data + financial reference data are processed for guests, couples, and vendors. Exact count: **`[TO CONFIRM]`** (see "number of data subjects with sensitive PII"). |
| Has **at least 250 employees** | **Likely NO** — headcount `[TO CONFIRM]`, but a sole-prop startup is almost certainly under 250. |
| Processing is **likely to pose a risk to the rights and freedoms of data subjects** | **Likely YES** — biometric face vectors, government-ID verification, minors/legacy roadmap, cross-border transfers. |
...
**Assessment:** at least one, and probably several, mandatory-registration triggers are met (risk to rights + non-occasional processing; and sensitive PII of ≥1,000 is likely once the count is confirmed). **Recommendation: register the PIC and its data processing system(s) with the NPC.** The DPO/owner should confirm the sensitive-PII data-subject count `[TO CONFIRM]` and the employee count `[TO CONFIRM]` to finalize which triggers formally apply, then file. Even if a specific numeric threshold turned out not to be met, the risk-based and non-occasional triggers independently support registration.
...
| Principal / registered office address | `[TO CONFIRM]` |
| Contact number | `[TO CONFIRM]` |
| Official email | `[TO CONFIRM]` (owner sign-up email on file: iscasasolaii@gmail.com) |
...
| NPC field | Value |
|---|---|
| Total number of employees | `[TO CONFIRM]` |
| Total number of data subjects | `[TO CONFIRM]` |
| Number of data subjects whose **sensitive personal information** is processed | `[TO CONFIRM]` |
...
## B.4 Does the system process **sensitive personal information**? — **YES**
...
|---|---|---|---|
| **Government-issued identifiers** | Government-issued ID of vendor primary representative (image), captured for identity + liveness verification | Vendors | Policy § 1.2, § 3.4, § 10 (Persona/Veriff/Onfido) |
| **Biometric data — face vectors** | 128-dimension face vectors generated from RSVP profile photo + optional pre-event upload + on-the-day check-in kiosk, used for per-event auto-tagging | Guests (and couples where they appear) | Policy § 1.4 |
...
| **Sanctions / PEP screening data** | Vendor business name + owner name screened against AMLC / ComplyAdvantage sanctions & PEP lists | Vendors | Policy § 3.4, § 10 |
...
> Note: some of the above (e.g., government IDs, financial account data) are "sensitive" and/or specially-regulated categories under RA 10173 / related law. Their inclusion here is for completeness of the NPC declaration; the DPO/counsel should confirm final categorization.
...
| Recipient / sub-processor | Jurisdiction | Data shared | Purpose | Lawful basis (per Policy) |
|---|---|---|---|---|
| **Supabase** (database) | **Singapore** | PII + transactional records | Application database (encrypted at rest) | Performance of contract |
| **Cloudflare R2** (object storage) | **APAC / PH region** | Media originals, face-vector index, documents | Media & document storage | Performance of contract |
| **Vercel** (application hosting) | `[TO CONFIRM jurisdiction]` (US-based) | Application traffic / runtime | Web application hosting | Performance of contract |
| **Resend** (email) | `[TO CONFIRM jurisdiction]` (US-based) | Recipient email + notification content | Transactional / notification email delivery | Performance of contract / legitimate interest |
| **Anthropic (Claude API)** | US | Vendor contract text | Contract Intelligence analysis (zero-retention mode) | § 12(b) legitimate interest; enterprise DPA |
| **Persona / Veriff / Onfido** | US | Vendor gov-ID image + selfie + liveness | Vendor identity verification | § 12(b) — vendor consent at registration |
| **AMLC API / ComplyAdvantage** | PH (AMLC) / UK (ComplyAdvantage) | Vendor + owner name | Sanctions / PEP screening | § 13(d) — compliance with PH AMLA |
| **DTI Database** | PH | Vendor DTI Business Name number | Business-registration validation | § 12(c) — necessity for contract |
| **OpenAI (GPT-4)** — V1.5+ fallback only | US | Vendor contract text (fallback) | Contract Intelligence backup | Same as Anthropic |
| **Suno** | `[TO CONFIRM jurisdiction]` | `[TO CONFIRM — data shared: AI music generation; confirm whether any personal data is transmitted]` | AI music generation for renders (Pakanta / template music) | `[TO CONFIRM lawful basis]` |

> The four US sub-processors are bound by SCCs or equivalent DPAs; the Anthropic workspace uses zero-retention API mode; Persona/Veriff/Onfido + AMLC screenings are batch-call-only with no ongoing storage (Policy § 10).
...
- Before filing, resolve **all `[TO CONFIRM]` fields**, confirm the RoPA (§ B.6) exists and is current, and have the DPO/counsel validate the sensitive-PII categorization (§ B.4) and the automated-decision answer (§ B.5).
...
| Filed with NPC (NPCRS) — reference no. | `[TO CONFIRM after filing]` | ________________ |

---

*Footer — This DRAFT establishes a compliant baseline ahead of counsel review. It is **not a substitute for legal review**. The actual NPC online registration of the data processing system(s) and the registration of the DPO's contact details are the DPO's / owner's filing actions. Finalize with the DPO (Indalecio Sacdalan Casasola II) and Philippine counsel before adoption or submission. · 2026-07-05.*
