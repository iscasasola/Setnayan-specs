...
| **Security measures** | RLS scoped to the event; the supplier sees only rows naming them; the coordinator's access is gated by the couple's opt-in money scope and the `coordinator_consent_money` control; proof images are signed-URL only |
