...
| 🔴 ⭑**UNCLAIMED · a planner reads the couple's budget TARGET** | ▶ **NOT STARTED — NO WAVE OWNS IT. ✅ UPGRADED FROM SOURCE-DERIVED TO PROD-VERIFIED 2026-08-24** (`pg_get_viewdef` · `pg_get_functiondef` · the table itself). **`events_host` carries the budget column and admits ANY accepted moderator; the function behind it has NO area filter while a sibling exists precisely to express budget access and is never consulted; the page adds no role check.** 🔴 **AND THERE IS A LIVE ROW — prod holds an ACTIVE `wedding_planner_external` on an event that HAS a budget.** 🚨 The view's own COMMENT says *"Guests, vendors and coordinators get ZERO rows"* — **false, and it is what an auditor reads.** ⚖ She is told two untrue things at once: a budget she should not see, and "Paid ₱0" on every supplier, because a refused read returns no rows rather than an error. ⛔ **A MISSING SCREEN, NOT A BROKEN PERMISSION** — the archetype already draws the DENIED state with its copy written. **Nobody should touch the policies over it.** 🔑 **AND IT IS NOT A "GATE WITH NO HANDLE" — IT IS SHARPER THAN THAT, MEASURED 2026-08-24.** `moderator_area_level(p_event_id, p_area)` is **live and used in 10 application files** (seating walkthrough, schedule stage notes, floor command, access requests). It is not unbuilt and it is not unused. **The budget page consults it ZERO times — and no file anywhere passes `'budget'` as the area**, even though the function's own comment in prod names budget explicitly: *"Budget never exceeds view in V1 (locked D1)."* ⇒ **The mechanism works on nine other doors and is skipped on the one guarding money.** ⚖ **THE FIX IS TO CALL IT** — gating **moderators only, never the couple**: fail-OPEN for the owner of the money, fail-CLOSED for a delegate. Narrowing the view instead would kill a working feature, because the coordinator legitimately needs that event for her other areas.
