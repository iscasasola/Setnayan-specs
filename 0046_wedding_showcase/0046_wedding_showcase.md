...
> - **FIRST SLICE SHIPPED 2026-06-13 (PR [#1349](https://github.com/iscasasola/setnayan-platform/pull/1349)).** The cross-wedding browse + per-showcase page are live at **`/weddings`** + **`/weddings/[slug]`** (as-built path is `/weddings`, not the spec's `/real-weddings/`), seeded with ONE clearly-labelled **sample** showcase (Maria & Juan). **The per-showcase detail (`/weddings/[slug]`) renders through the SAME `EditorialContent` as a real wedding's Phase-4 editorial** — fed a `sampleEditorialData()` fixture via a `loadEditorialData` sentinel (PR [#1350](https://github.com/iscasasola/setnayan-platform/pull/1350)), so the sample FOLLOWS the editorial format automatically (no parallel layout to drift). The index (`/weddings`) is now **DB-driven + consent-gated** (`lib/showcase-db.ts` `loadPublishedShowcases` — PR [#1353](https://github.com/iscasasola/setnayan-platform/pull/1353)): it surfaces real published editorials (each linking to the couple's canonical `/[slug]`) when any qualify — `event_type='wedding'` + public slug + past the T+30d grace window + a couple member's `users.public_summary_consent_at` (the RA 10173 gate) — and falls back to the sample otherwise, so the sample yields automatically the instant a real wedding is uploaded. The sample (`isSample:true`) carries a visible "Sample showcase" label + a "real editorials begin December 2026" line and has NO real PII. **STILL UNBUILT:** the dedicated showcase TABLES (the as-built browse uses the existing `users.public_summary_consent_at` gate instead of `wedding_showcases` / `_vendor_credits` / `_captures` / `_product_credits` / `_facets` / `_vendor_claims`), the vendor showcase-submission flow (the in-dashboard consent toggle SHIPPED via PR [#1354](https://github.com/iscasasola/setnayan-platform/pull/1354) — couples opt in/out anytime at /dashboard/[eventId]/website/privacy; consent capture is also the signup checkbox), faceted per-combination SEO landing pages, vendor-portfolio + product auto-population, and `wedding_guest_reviews` / `vendor_event_feedback`. The canonical real model is unchanged: **consent-gated, DB-driven editorials publish from couples' own event pages at T+30d (0046/0002 Phase 4; first = the founder's Dec 18 2026 wedding → ~Jan 2027)** and merge in alongside/replacing the sample.
...
> - Planner-SKU references should read **Setnayan AI ₱1,499**, not Setnayan Concierge ₱4,999.
>
