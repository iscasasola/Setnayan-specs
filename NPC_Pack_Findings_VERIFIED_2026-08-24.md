...
| 14 | ✅ **MOSTLY BUILT 2026-08-24 — DO NOT REBUILD.** Shipped: withdrawal blurs-and-keeps instead of vetoing ([#4747](https://github.com/iscasasola/setnayan-platform/pull/4747), verified in prod) · the guest's own FaceBlock switch · the notification when the couple switches it off ([#4749](https://github.com/iscasasola/setnayan-platform/pull/4749)). ⏭ **Still owed, and it is a BUILD, not a decision:** ruling 1 — extend blur to the **public event page and shared pool** — scoped in `DECISION_LOG.md` 2026-08-24 (it needs blurred *display/thumb* derivatives, not a filter change). Plus correcting the pack and `/privacy`, which now UNDERSTATE what the code does. **Do not re-ask the four rulings.** | **engineering** (already ruled) |
