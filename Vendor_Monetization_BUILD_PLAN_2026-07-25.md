...
## ⏱ BUILD PROGRESS (updated 2026-07-25 — 11 PRs, all flag-dark; #3702 + #3710 landing)
**Merged to `main`:**
...
- **#3705** — 🔴 **LIVE SECURITY FIX (migration `20271002456914`): vendors could SELF-GRANT the paid add-ons.** `vendor_profiles_owner` is `FOR ALL` with no column scoping and **RLS is ROW-level, never COLUMN-level**, so any column the guard trigger doesn't name is vendor-writable via PostgREST. One PATCH bought a free branded 3D booth in couples' LIVE plans, a free Vendor AI window, and an infinitely re-armable "one-time" trial. **Proven both ways** (6/9 db tests fail without the fix) · **VERIFIED APPLIED in prod** · **forensics CLEAN — zero rows, never exploited.**
- **#3706** — **AI ladder SCHEMA** (migration `20271003111715`): `vendor_profiles.ai_addon_level` + the `vendor_ai_addon_advanced` SKU seeded **`is_active=FALSE`**. Verified applied in prod (column + default `'basic'` + CHECK + guard coverage + SKU inactive at ₱3,000 + backfill clean).
- **#3702** *(landing)* — AI + Deep Search onto the tier band. ⚠ the band is **injected as `cyclePricePhp`**, never as the resolver's output — both resolvers return ₱0 *before* reading it, so replacing the output would bill the AI trial and delete the Pro+ free search.
- **#3710** *(landing)* — **AI ladder CODE**: `lib/vendor-ai-level.ts` (pure) + `lib/vendor-ai-ladder-flag.ts` + a 2nd `EXACT_HOOKS` entry sharing the same activation hook. ⚠ the level demotion on refund is a **SEPARATE** step because `deactivateVendorAddonWindow` returns EARLY when a later cycle owns the window — exactly the refunded-Advanced-over-live-Basic case — and it **re-derives** rather than blind-clears.
...
1a. **⚠ AI ADVANCED DOES NOTHING YET — the spine is built, the product is not.** The `mode='smart'` / `voice_profile` / `vendor_reply_templates` schema has existed since `20270822679405`, but a full survey confirmed **ZERO app code reads any of it**. #3706 + #3710 shipped only the ENTITLEMENT SPINE (sell · track · activate · revoke a level). The capabilities a vendor would be paying ₱2,500–3,000 for — **voice-match · precomputed natural phrasing · reply-in-couple's-language · lead analytics · higher cap** — are **unbuilt**. That is why the SKU ships `is_active=FALSE`; do NOT flip it until they exist. **Next real engineering:** voice-match + the precompute path (generate phrasings once per voice/catalog edit → serve deterministically, keeping per-reply cost ₱0 per [[Rule 1]]).
