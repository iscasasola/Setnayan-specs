...
- **Chat** (`chat_threads` / `chat_messages`): text-only, append-only, no attachments. **No** `archived_at` / retention / soft-delete columns. `chat_threads.updated_at` already tracks last activity (free `last_message_at`).
