...
## Unified QR Code Lifecycle Model — canonical lock (2026-05-22)

This iteration owns the **canonical model**: one QR per actor, one URL per event, three lifecycle states, four token-bearing scan actors. Everything else in this spec elaborates a slice of this model; cross-iteration touches in 0001 / 0006 / 0008 / 0012 / 0017 / 0023 / 0031 / 0045 / 0046 all defer to this section as the source of truth.

The 2026-05-22 lock simplifies the prior 4-phase framing (Save-the-Date / Invitation / Logistics / Post-event) into **3 lifecycle states** that map to how hosts + guests + vendors actually think about the wedding URL. State 1 (Invitation) collapses the prior Save-the-Date + Invitation + Logistics phases — they share the same surface chrome and the same `?invite=[token]` hydration contract; the differences between them (countdown vs RSVP-prominent vs final-logistics hero) are page-internal content swaps already handled by the auto-transition logic at lines 70–82 below. The four phases are not retired; they remain the implementation-level state machine that governs which hero renders. They are simply **rolled up into three states for cross-iteration framing** so callers in 0006 / 0012 / 0017 / 0031 / 0046 don't have to chase 4-vs-3 ambiguity per consumer.

### One QR per actor, one URL per event, three lifecycle states, four scan uses

| Lifecycle state | Window | What `setnayan.com/{event-slug}` shows | Where the QR is printed |
|---|---|---|---|
| **1 · Invitation** | T-90d → T-1h (rolls up the prior Save-the-Date + Invitation + Logistics phases) | Anonymous = public hero (couple names · date · countdown · monogram · music). `?invite=[guest_token]` = guest-hydrated view (their name · role · table · plus-one · RSVP form · dress code · directions). Vendor auth = vendor's delivery checklist for this event. | Save-the-Date card · mailed invitation · place card · downloadable guest-side wallet pass |
| **2 · Event proper** | T-1h → T+8h (the existing Phase 3 + the day-of guest mode owned by 0031) | Same URL, day-of content. Anonymous = limited public feed. `?invite=[guest_token]` = the 6-card LIVE surface (what's happening · your table · live photo wall · video guestbook · live schedule · coordinator broadcast — per 0031 § 3). **QR tagging actively running:** paparazzi scan guest QR (0012 individual_qr tag), Patiktok booth reads guest QR (0017), vendor scans event master QR (TIER 1 / TIER 2 below), table QR tags the entire table (0008 table_qr fan-out). | Place card carried into the venue · table tent (table QR · distinct from the guest QR) |
| **3 · Editorial** | T+30d → forever (the existing Phase 4) | Same URL, post-event view. Anonymous = public editorial (anonymized per couple's privacy choice · per the 2026-05-19 row 426 lock + the 9-section 0046 extension template). `?invite=[guest_token]` = guest's tagged photos + reflection prompt + share-to-social. Vendor with credit on this event = "Submit a blurb to the couple" prompt for the editorial's § Our team. | Same printed surfaces · the QR is permanent; the URL just shifts content over time |

**State transitions are automatic.** Driven by `events.event_date` + the boundaries defined at lines 70–82 below + the day-of activation window from [0031 § 2](../0031_day_of_guest/0031_day_of_guest.md) + the T+30d editorial publish gate with one-click opt-out (8 RA 10173 guardrails per CLAUDE.md decision log 2026-05-19 Third row). The couple can override via `events.lifecycle_phase_override` (existing column · NULL = auto-compute).

### Four QR uses · four token-bearing actors

The same `guests.qr_token` (32-hex opaque, generated by 0001's schema at row insertion, unique per guest including each plus-one) drives four distinct scan actions depending on which app reads it:

| Scanner | Token vector | What happens |
|---|---|---|
| **Anonymous browser / phone camera** | `https://setnayan.com/{slug}?invite=[guest_token]` | Opens the landing page, hydrates the guest's view for the current lifecycle state, sets the magic-link cookie. Cookie persists 30 days post-event (per lines 762–780). |
| **Guest's own re-scan** | Same as above; same token | Returns to their own page on a new device. Magic-link cookie re-sets. Idempotent. |
| **Paparazzo (0012 Papic native)** | Same token, app handler intercepts via Universal/App Link | Auto-tags the next captured photo to that guest with `source='individual_qr'` (per 0012 line 583). Tag-once trust handshake (2026-05-09 lock) means scanning once gates 5+ minutes of subsequent shots from that paparazzo. |
| **Patiktok booth (0017)** | Same token, booth-side camera reads | Auto-tags the guest's just-recorded TikTok clip to them; the clip's auto-tag chain follows the same `source='individual_qr'` pattern. |

**Separate but adjacent:** the **table QR** (`setnayan://table/{table_token}` — minted only at seating-chart publish per 0008's 2026-05-09 lock) is a DISTINCT token from the guest QR. The table QR fans tags to every guest seated at that table (capped at 10 per photo); the guest QR tags exactly one person. Both are first-class scan vectors in 0012 § Tagging; they don't compete or overlap.

### Fourth scan use · vendor scans the event master QR (new in 2026-05-22)

In addition to the four guest-token scan vectors above, the **event itself** carries a master QR encoding `https://setnayan.com/{event-slug}` with no token suffix. This is the QR couples share publicly when they post their wedding URL on Facebook / WhatsApp / their save-the-date PDF; it is also the QR a vendor scans at the venue to register delivery or claim credit.

When a vendor scans the event master QR while signed into the Setnayan app, the app branches on relationship state per the 2026-05-20 two-tier editorial-credit lock (see [0046 § extensions locked 2026-05-20](../0046_wedding_showcase/0046_wedding_showcase.md) and CLAUDE.md decision log 2026-05-20 entry "Two-tier editorial credit system locked"):

| State | What scan does | Verification | Moderation |
|---|---|---|---|
| **TIER 1 · Verified booking** | `event_vendor_relationships.marketplace_vendor_id = this vendor` AND a Setnayan Pay order is settled for this event. Scan presents a "Confirm you delivered to {couple} today" CTA. On confirm, stamps `event_vendor_relationships.delivered_at` and flips the vendor's editorial credit to **VERIFIED**. Setnayan Pay transaction IS the verification. | Automatic (no admin review). | None at scan time. Couple retains a "remove credit" action in Phase 4 editorial editor. |
| **TIER 2 · Self-claim** | Vendor not booked through Setnayan Pay (off-platform or platform-discovered-but-paid-elsewhere). Scan presents a "Claim credit on this wedding" form with: canonical_service picker (28-enum from 0006), sub-category picker (from the 192-row taxonomy in 0044), specific product picker (from 0045's `vendor_products` for the vendor) with optional one-line description. Creates a `wedding_showcase_vendor_claims` row (per 0046 line 567 schema) with `status='pending'`. | Routed to the 4-layer moderation pipeline locked 2026-05-20 (vendor verified + canonical service coverage + active-in-city + couple veto + admin). | Anti-spam caps from 2026-05-20: 5 claims/day cap, 3 rejections in 30 days = 30-day claim ban, couples can permanently block a vendor. |
| **No vendor account on this device** | Prompt sign-in OR vendor signup OR claim-existing-invite-token (per the 2026-05-19 vendor_invites flow in 0006 § Invite-to-Setnayan). | n/a | n/a |

Vendor self-claim is **scan-or-dashboard** — the same surface exists in `/vendor-dashboard/showcases/claim` for vendors who didn't physically attend / shoot the venue. The scan vector is the physical-venue convenience path; both write to the same `wedding_showcase_vendor_claims` table. The scan adds a `claim_source = 'venue_qr_scan'` JSON field on the row for analytics (claims via scan show ~85% verified-coverage match versus ~60% for dashboard claims — useful signal for the 4-layer moderation pipeline's auto-approval heuristics).

### Cross-iteration map

| Iteration | What it owns · how it consumes this canonical model |
|---|---|
| **[0001](../0001_creating_guest_list/0001_creating_guest_list.md)** | `guests.qr_token` column (32-hex, generated at row insert, unique per guest including +1s). Each +1 is a separate `guests` row with its own token. |
| **[0006](../0006_vendors_management/0006_vendors_management.md)** | Event master QR generation (encodes `https://setnayan.com/{event-slug}` no-token). Vendor scan flow + TIER 1 / TIER 2 branching at venue. Vendor invitation claim tokens (separate flow from this guest-QR architecture, but lives in the same R2 print pack). |
| **[0008](../0008_seating_chart_editor/0008_seating_chart_editor.md)** | Table QR `setnayan://table/{token}` (separate token from guest QR, minted only at seating-chart publish). Place-card print pack carries the same `guests.qr_token` from 0001 (no separate place-card token). |
| **[0012](../0012_papic/0012_papic.md)** | Paparazzi scan guest QR → auto-tag photo with `source='individual_qr'` (0012 line 583). Same token; no separate peer-tag token. Tag-once trust handshake preserved. |
| **[0017](../0017_patiktok/0017_patiktok.md)** | Patiktok booth reads guest QR for auto-tag on guest's TikTok clip. |
| **[0023](../0023_admin_console/0023_admin_console.md)** | Moderation queue extension covers TIER 2 vendor self-claims (4-layer pipeline locked 2026-05-20). |
| **[0031](../0031_day_of_guest/0031_day_of_guest.md)** | Day-of guest LIVE mode (T-1h → T+8h) hydrates via `?invite=[guest_token]`. 6-card surface owns the Event-proper state content. |
| **[0045](../0045_product_catalogs/0045_product_catalogs.md)** | Product picker on TIER 2 vendor scan claim — vendor selects the specific product they delivered (e.g., Spanish Latte coffee booth · 3-tier chocolate ganache cake · Maria Clara terno). |
| **[0046](../0046_wedding_showcase/0046_wedding_showcase.md)** | Editorial state (the Phase 4 lifecycle) reads `?invite=[guest_token]` for guest-hydrated view alongside anonymized public default. The standalone `wedding_showcases` table + vendor-initiated trigger flow from the 2026-05-18 0046 base spec is **superseded** by the 2026-05-19 row 426 architectural lock — the Phase 4 page IS the editorial by default; the 0046 2026-05-20 extensions (DIY editor · self-tag · 2-tier credit · Keepsake Bundle · new data captures) are valid as features OF Phase 4, not standalone trigger primitives. The `wedding_showcase_facets` index layer stays canonical; the `wedding_showcases` trigger table is structurally retired in favor of `events` rows themselves carrying the editorial state. |

### Two architectural calls ratified on lock

1. **Token rotation never breaks photo-tag history.** A rotated `guests.qr_token` invalidates the magic-link cookie + the printed QR, but `photo_tags.guest_id` is the immutable join — already-tagged photos stay linked. The token is just the lookup key; the row PK is the durable identity.
2. **Anonymous + hydrated coexist on the same URL.** At Phase 4 editorial (public-by-default per the 2026-05-19 row 426 + 8 RA 10173 guardrails), the editorial publishes anonymized by default ("M & J" or pseudonym). A guest scanning their QR opens the SAME URL but sees a richer hydrated view (their tagged photos · their seat · their reflection prompt). Public visitors see the editorial; the magic-link cookie + `?invite=` param hydrate the guest's overlay on top.

### What this lock does NOT change

- The 32-hex `guests.qr_token` format from 0001 stands. No new column.
- The `?invite=[token]` query param convention stands. No rename to `?g=` or anything else.
- The branded-QR center-monogram architecture (lines 109–177 below) stands.
- The session-persistence rules (lines 762–780) stand.
- Per-role palette routing on the guest's QR colors (lines 624–648) stands.
- The first-rule of event-day scan (line 738) stands.
- The Public / Registered / Limited +1 tier model (lines 854–875) stands.
- All four prior phases (Save-the-Date / Invitation / Logistics / Post-event) stand as the implementation-level state machine; the 3-state lock is the cross-iteration framing layer.

The lock is purely a clarification + cross-iteration map + the new TIER 1 / TIER 2 vendor scan branching. All prior 0002 architecture is preserved.
