...
| Slot per day = **vendor-declared daily booking CAPACITY per service** (owner-defined 2026-06-07: e.g. 2 photobooths → 2 bookings/day; tier caps the max declarable) | ✗ | 1 (flat) | 3 (flat — **NOT** time-bounded) | Unlimited **+ time-bound slots** | ⛔ build pending. **Owner refinement 2026-06-07:** time-bounding is **ENTERPRISE-ONLY** (Pro is a flat 3/day, no am/pm). Enterprise can **plot named time slots, each with its own allotted capacity** (use case: a hotel scheduling its different rooms across the day). Plain capacity: `vendor_services.daily_capacity` (capped by tier on save) + per-service same-date count in `finalizeVendor`. Enterprise time-bound = a SEPARATE larger feature: a per-service time-slot table (window + allotment) + booking-to-slot binding. `slotsTimeBounded` = true for ENTERPRISE only. |
...
> - **NEW — subscriptions bundle free tokens:** Pro +30 (28d) / +300 (annual); Enterprise +100 (28d) / +1,000 (annual). Granted on subscription activation/renewal → belongs to Phase D (no subscription-payment flow exists yet).
