...
> 🏷 **THE ANNUAL DISCOUNT IS NOW 20%, NOT ~23% — AND THE ROW TITLES SAID SO OUT LOUD.** Annual moved from 28-day × **10** (13 cycles for 10) to × **10.4**. Three row TITLES carried the old figure as a customer-facing promise — *Solo … (Annual · save 12 weeks)*, *Pro / Enterprise … (Annual · save ~23%)* — so shipping the new prices under them would have advertised a discount we no longer give. All three now read **save 20%**, which is EXACT on all three (13,000 vs 10,400 · 32,500 vs 26,000 · 130,000 vs 104,000). The same claim was hardcoded in two rendered surfaces (the annual badge on the subscription cards, the cycle-toggle hint) and both moved in the same change. ⚠ **Custom's annual is still 10× (~23%)**, because the owner did not rule on it — the three tiers and Custom now discount annual differently. Flagged, not changed.
>
...
>
> ⚖ **CUSTOM STAYS LIVE, ABOVE ENTERPRISE — ruled retired on 2026-08-27 and REVERSED the same day, before anything was applied.** 🔑 **THE DURABLE LESSON: AN `is_active` FLIP IS NOT A RETIREMENT WHEN A CODE-SIDE FALLBACK EXISTS.** `lib/vendor-custom-catalog.ts` reads the Custom rows with `.eq('is_active', true)` and then substitutes a **hardcoded literal** for any row that comes back missing — its own docblock already said, in terms, that deactivating a row *"is not a retirement"* and that the axis *"keeps quoting, at the same price, with the catalog saying it is off."* The flag would therefore have produced a HALF state: `/vendors` and the homepage stop showing Custom (they filter `is_active`, and `customFrom` has no peso fallback), while the configurator — still linked from `/vendor-dashboard/subscription` — keeps quoting and keeps selling it. A tier invisible to shoppers and fully buyable by anyone already inside. Owner, shown that: a supplier must see exactly what they can buy, and he would rather Custom stay genuinely on sale than ship a tier that looks retired and is not. **Before retiring any catalogue row, grep for a reader that supplies a literal when the row goes missing.**
>
...
> 🚨 **THREE BACK DOORS UNDER TODAY'S PRICES, ALL CLOSED — AND THE RULE GENERALISED.** Besides the Custom base below, two more hardcoded literals shadowed rows this same price sheet moved: `BRANCH_FEE_PHP` still said ₱999 against a row going to ₱1,000, and `VENDOR_3D_BOOTH_FALLBACK_PHP` still said ₱1,500 against a row going to ₱2,500. Each would have charged the OLD price on any failed catalogue read while the catalogue looked correct. Both fixed, and the rule is now enforced generally: `apps/web/tests/db/fallback-prices-match-the-catalog.db.test.ts` scans `lib/` for fallback/SKU pairs and fails the build when any literal disagrees with its live row. 🔑 **A file it cannot pair is a FAILURE, not a skip** — it must be named with a reason — because a silently-skipped file is how a guard stops covering what it was written for.
>
> 🚨 **AND A BACK DOOR UNDER THE LADDER, CLOSED THE SAME DAY.** Raising Custom's catalogue base to ₱11,000 was not enough on its own: `CUSTOM_UNIT_PRICE_FALLBACK` in `lib/vendor-custom-catalog.ts` hardcoded `base: 8999`, and `fetchCustomUnitPrices` substitutes that literal whenever the row is inactive or unreadable — so a single failed read would have quoted ₱8,999 against Enterprise at ₱10,000 and **restored the inversion through a door no catalogue check can see**. The literal is now ₱11,000 (and its `branch` axis ₱999 → ₱1,000), and the guard was extended to assert the FALLBACK is above Enterprise too and that every fallback axis agrees with the live catalogue. 🔑 **A second copy of a price is a second place for it to be wrong; if you cannot delete the copy, make a test compare it.**
>
...
- **Enterprise ₱10,000 (~$172)** — still below a *single* Knot listing, adds 10 seats + API + everything. _(Was ₱8,000 / ~$138 until the owner's 2026-08-27 price sheet. Custom sits above it, unchanged.)_
