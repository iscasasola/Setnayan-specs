...
| ~~W5-C · Who is in my event?~~ | ✅ **DONE** 08-24 · 4 PRs — #4799 · #4801 · #4803 **MERGED AND SERVED** (prod self-reports `5d4facb`, #4803's own merge commit) · #4807 (AP-11) open, auto-merge armed. 🔴 The budget item shipped FIRST and ALONE and the brief understated it — the delegate could also **CHANGE** the ₱930,000 target, through the admin client. 🪤 Three surfaces print that figure, not one. | Opus 5 | medium | 5 |
...
| ~~UNCLAIMED · a planner reads the couple's budget TARGET~~ | ✅ **FIXED 2026-08-24 · #4799 "the budget refuses the delegate it was never shared with".** Raised by an audit, verified in prod against a LIVE external-planner row, scheduled into W5-C, shipped the same day. #4801 fixed the coordinator's dead-end control alongside it. | — | — | 5 |
...
| 🔴 ⭑**UNCLAIMED · the owner's blur ruling is only one-third honoured** | ▶ **NOT STARTED — parts 2 and 3, genuinely unbuilt rather than overlooked.** ✅ **VERIFIED HERE:** `papic_capture_needs_blur` exists and has **ZERO application callers** — it appears only in a db test and two changelog fragments. **Every public read still serves the UNBLURRED copy**, so of his 2026-08-17 ruling only the **venue wall** is honoured. 🔢 Safe today by arithmetic — 14 photos, 0 needing blur — **not by design**. ⛔ Not built by W6 deliberately: it changes what a person sees on a public page. Scoped in `WHATS_NEXT_Blur_Parts_2_And_3_2026-08-25.md`. 🔑 **Seventh gate with no handle, and the first one sitting on an owner ruling about a guest's face.** | — | — | — |
| ~~UNCLAIMED · 98 loading screens still draw the retired page title~~ | ✅ **DONE 2026-08-25** — and it went further than the count: **27 more screens were borrowing a loading title from a NEIGHBOURING page**, and the sweep's own audit caught a regression it had caused. | — | — | — |
| ⭑**UNCLAIMED · THE GALLERY ARCHETYPE'S THREE REAL SUBJECTS** | 🔴 **STILL OPEN — RE-SCOPED 2026-08-25 (W5-E) after my own first closure of it proved too broad.** `alaala` is indeed not the gallery — **and neither is `galleries/`.** The archetype's dashboard chip is `/dashboard/[event-id]/**gallery**`, SINGULAR, and **no such route exists**; `galleries/` is a hub of three source rows (Papic · Live Studio · your own photos) while the archetype draws an obsidian photo mosaic with a lightbox and per-tile camera credit, and calls its subject *"The Alaala memory hub"*. ❌ **AND MY ONWARD POINTER WAS WRONG: the Papic RECAP renders ZERO images** — it is a counter strip, a publish switch and a Drive nudge. Following that sentence would have ported a page of counters and reported the gallery done. ❌ **The disqualifier was unsound too:** I ruled the archetype out because its chip `/dashboard/[event-id]/`**`gallery`** (singular) names a route that does not exist — measured, **TWO of its four chips name routes that never existed**, so a chip here is a **LABEL, not an address**. 🔑 **THE THREE REAL, SHIPPED SUBJECTS, ALL UNPORTED** (measured on `origin/main`: zero `#17160F`, zero per-tile camera credit in any of them; only the first has a lightbox and it is clips-only): `studio/papic/_components/papic-gallery-grid.tsx` (the couple's own wall of photographs) · `app/[slug]/_components/your-photos-widget.tsx` (what a guest sees of their own) · `app/[slug]/_components/live-wall-block.tsx` (the day-of wall). **None of the three needs an owner ruling.** ⏭ The ONE genuine design/owner question is narrow: whether a hub-of-links page like `galleries/` belongs to that archetype at all. ⏭ Still unclaimed and untouched: the couple's vendors LIST route answers to no archetype and stays under its own locked spec. | — | — | — |
...
- 🎨 **CHECK A COLOUR AGAINST THE SURFACE IT LANDS ON, NEVER AGAINST WHITE — VERIFIED BY
  CALCULATION 2026-08-24.** This repo's canonical TEXT gold `#8A6B39` measures **4.95:1 on white
  and 4.42:1 on the `#F4F2EC` card**. It PASSES on one surface and FAILS the 4.5:1 floor on the
  other, by 0.08. **Nothing about the token tells you which**, so "is this the approved gold?" is
  the wrong question and "what is it sitting on?" is the right one.
  🔑 Gold has almost no headroom in this product — a related note already records that ANY tint
  under it fails, including the kit's own faint ink wash. Assume a gold on a tinted card is failing
  until you have computed it.
...
### ~~W5-C · Who is in my event?~~ · ✅ **DONE 2026-08-24 — do NOT rebuild any of it**

> **4 PRs. [#4799](https://github.com/iscasasola/setnayan-platform/pull/4799) · [#4801](https://github.com/iscasasola/setnayan-platform/pull/4801) · [#4803](https://github.com/iscasasola/setnayan-platform/pull/4803) are MERGED AND SERVED** — production self-reports `5d4facb`, which is #4803's own merge commit, so they are shipped rather than merely merged. [#4807](https://github.com/iscasasola/setnayan-platform/pull/4807) (AP-11) is open with auto-merge armed. **No migration in any of them.** Full row: `DECISION_LOG.md` 2026-08-24.
>
> 🔴 **THE BUDGET REFUSAL WENT FIRST AND ALONE — and the brief understated it.** Prod carries a live accepted `wedding_planner_external` (`checkout:false`) on an event whose target is ₱930,000. She could read it **and change it**: `updateEventMatchCriteria` authorises on "member (couple/coordinator) OR accepted moderator" and wrote the column through the **ADMIN** client. A read leak was the smaller half.
> 🔑 **NOTHING INVENTED A POLICY.** `'budget'` has been a declared delegate area, **defaulted OFF**, since `20261129000000`, and `moderator_area_level` is called correctly in **nine** other files. This was the missing **call site**, not a missing mechanism. ⛔ **No RLS, grant or view touched** — narrowing `events_host` would quietly kill a working feature.
> 🪤 **A FIX AT ONE SITE WOULD NOT HAVE BEEN A FIX: THREE surfaces print that figure** — `/budget`, the **event home tile** ("committed of ₱930,000", the first screen she lands on) and **Personalization**, which had *no membership gate of any kind*. The guard's file list is DERIVED from the code.
...
### ~~⭑ W5-E · Two surfaces nobody claimed~~ · ⚖ **ITEM 2 DONE 2026-08-25 · ITEM 1 RE-SCOPED AND STILL OPEN** — do NOT re-run the budget half

> 🛑 **READ THIS BEFORE THE PROMPT BELOW — AND READ MY OWN CORRECTION UNDER IT.**
>
> 🔴 **ITEM 1 IS NOT DONE. I CLOSED IT, AND AN ADVERSARIAL AUDIT OF MY OWN WORK SHOWED THE
> CLOSURE WAS TOO BROAD.** What stands: `galleries/` really is not the Gallery archetype's
> subject. What does not: I disqualified the archetype because its chip
> `/dashboard/[event-id]/`**`gallery`** (singular) names a route that does not exist —
> **measured, TWO of its four chips name routes that never existed**, so a chip in these
> prototypes is a **LABEL, not an address**. And my onward pointer named the Papic **recap**
> as the real grid; **that route renders zero images.**
> 🔑 **The archetype's three real, shipped subjects — all unported, measured (zero `#17160F`,
> zero per-tile camera credit):** the couple's **Papic gallery grid**, the guest **"your
> photos"** widget, and the **day-of live wall card**. None of them needs an owner ruling.
> Full row: `DECISION_LOG.md` 2026-08-25 (the correction row, not the first one).
>
> ⚠ **The original banner is kept below, struck where it was wrong.**
>
> 🛑 ~~ITEM 1'S PREMISE DID NOT SURVIVE MEASUREMENT.~~
