...
4. **Recording handoff (answers "how do hosts download their live?").** After the event, Setnayan **pulls each stream's VOD** from the pool channel and delivers it to the couple — download on their dashboard + into their **Alaala** gallery (optionally re-published to a channel they connect). ~~The channel is then wiped + returned to the pool.~~ **RETIRED 2026-08-31 — the archive is NEVER wiped** (owner ruling; `09_Panood_Feature_Specification.md` § 6 wins: indefinite retention). Wiping would delete a wedding. Release of a pool channel stays an explicit admin act and destroys nothing. Matches the existing "auto-archive → couple downloads" pattern.
