# Setnayan V1 Developer Specification

> ## WARNING: AS-BUILT CORRECTION — 2026-06-07 (reconciled to live site + origin/main)
> **This doc is HISTORICAL/REFERENCE.** Authoritative current state = the live site (www.setnayan.com) + shipped code + `AS_BUILT_GROUND_TRUTH_2026-06-07.md`. This 2026-05-07 doc predates almost every locked V1 decision and was already flagged RETIRED below; the items most likely to mislead a reader:
...
> - **Tiers/SKUs:** Essentials/Premium/Pro Event wedding tiers are retired; the planner SKU is **Setnayan AI ₱3,999** (not "Kasalan"/"Concierge"/"Today's Focus"); customer token wallet RETIRED; a **vendor token economy** (burn-on-answer, 100 free founder tokens) is LIVE.
...
> **⚠ RETIRED 2026-05-12.** This document is the V1 developer specification as it stood on 2026-05-07. It predates the token-wallet retirement (2026-05-11), the Setnayan brand rename (2026-05-12), the charm-pricing decision (2026-05-12), the free-vendor-launch model, the apply-then-pay payment flow, and many other locked decisions. **Do not implement from this document.** The active V1 source-of-truth is the per-iteration `.md` specs in folders 0000–0033 plus `CLAUDE.md` (decision log) plus the Vendor Agreement. The active payment flow is documented in iteration 0034 (cart + screenshot reconciliation). This file is preserved for historical context and migration mapping only.

**Document version:** 1.0
**Last updated:** 2026-05-07
**Status:** RETIRED (see banner above). Was: Master build spec for V1.0
**Audience:** Historical reference only — engineering team should consult per-iteration specs + CLAUDE.md

This document is the canonical, executable specification for the Setnayan V1 build. It supersedes informal notes and Slack decisions. Every section is intended to be unambiguous enough that an engineer (or a Claude Code instance) can act on it directly. Where decisions are still pending, they are explicitly listed in Section 13.
...
**Setnayan** — Filipino-Catholic-first wedding event platform.

### One-paragraph description

Setnayan is a Filipino-Catholic-first wedding event platform. Couples plan their wedding (budget, timeline, guests, mass roles, seating, vendors). Guests get a QR-bound identity that links them to RSVP, a personal photo album, and the wedding-day photo upload feed. Vendors register, list packages with rich inclusions and Setnayan-exclusive pricing, and respond to couple inquiries through gated in-app messaging. Coordinators manage multiple weddings from a single dashboard, with referral-based commission tracking through the Integrated Coordinator program. A Setnayan Staff role oversees vendor approvals, certifications, and disputes. The whole platform is unified by a single canonical event URL (e.g. `setnayan.com/maria-juan-2026`) that renders role-appropriate views and is reachable from any QR code on any printed wedding asset.

### V1.0 scope

The full V1.0 feature scope is enumerated in Section 7. At a glance:

- 5 user roles: couple, guest, vendor, coordinator, Setnayan Staff (plus stylist as a coordinator subtype)
...
- Kasalan AI assistant (Claude Sonnet)
...
- Vendor marketplace transactions (Setnayan holding deposits in escrow) — V2
...
- Cloudflare account (R2 bucket + DNS for `setnayan.com`)
...
|   `-- admin/            # Internal admin tool (Setnayan Staff only) - separate Next.js app
|-- packages/
|   |-- ui/               # shadcn/ui components + custom Setnayan components
...
Use **pnpm workspaces + Turborepo** for monorepo management. Turborepo gives us cached builds and parallel task execution. The two Next.js apps (`web`, `admin`) share the same Supabase project but are deployed as separate Vercel projects with separate domains (`setnayan.com` and `admin.setnayan.com`).
...
R2_BUCKET_NAME=setnayan-uploads
...
REKOGNITION_COLLECTION_PREFIX=setnayan-event-
...
NEXT_PUBLIC_ADMIN_URL=https://admin.setnayan.com
NEXTAUTH_SECRET=
...
Every account in the system: couple, guest, vendor, coordinator, Setnayan Staff.
...
`qr_token` is a 32-char URL-safe random string. Each guest's QR code resolves to `setnayan.com/[event-slug]?invite=[qr_token]`.
...
create table vendor_packages (
  ...
  setnayan_price numeric not null,
  next_year_regular_price numeric,
  next_year_setnayan_price numeric,
...
Each placement is a Setnayan Exclusive commitment — where the vendor agrees to show the event QR.
...
create table coordinators (
  ...
  is_setnayan_integrated boolean not null default false,
...
| Setnayan Staff | Admin home | Vendor Approvals, Certifications, Disputes, System Health, Financials | Read all events; can create their own as a couple |

### 5.3 Account Creation Flows

**Couple signup:**

1. Land on `setnayan.com` → click "Plan your wedding"
...
4. Setnayan creates an `events` row, generates a slug from the names + year, and seeds the default checklist (21 categories from Section 7 of `05_Default_Filipino_Wedding_Template_v1.docx`)
5. Redirect to dashboard

**Vendor signup:**

1. Land on `setnayan.com/vendors` → click "Become a Setnayan Vendor"
2. Application form: business name, category, location, portfolio
3. Submit → Setnayan Staff review (1–2 days)
...
2. QR deep-links to `setnayan.com/maria-juan-2026?invite=[token]`
...
Single canonical event URL: `setnayan.com/[event-slug]`
...
`setnayan.com/dashboard` (without event slug) → "My Events" home: cards for every event the user is connected to, tap a card to enter that event's role-appropriate view.
...
### 6.1 Public Event Landing Page (`setnayan.com/[slug]`)
...
### 6.2 Couple Dashboard (`setnayan.com/dashboard`)
...
### 6.3 Vendor Browse (`setnayan.com/vendors`)

**Layout:**

- Top filter bar: category, location, budget range, free-transport zone toggle, Setnayan Recommended toggle
- Grid of vendor cards (3-column desktop, 1-column mobile)
- Each card: cover photo, business name, category, rating, Setnayan Recommended badge if applicable, "Free transport in your area" badge if applicable, starting price
...
### 6.4 Vendor Profile (`setnayan.com/vendor/[slug]`)

**Layout:**

- Hero: cover photo + business name + Setnayan Recommended badge + rating + location
- Stats row: years in business, response time, bookings YTD
- Tabs: Packages | Portfolio | Reviews | Contact
- Each Package tab card shows: name, regular price (struck), Setnayan price, Setnayan Exclusive badge, Inclusions, What's NOT Included, Additional Costs, Capacity, Lead time, Transport zone, Recommended For tags, sample photos, "Inquire about this package" CTA
...
### 6.6 Vendor Dashboard (`setnayan.com/vendor`)
...
### 6.8 Coordinator Dashboard (`setnayan.com/coordinator`)

**Layout:**

- Header: name + Setnayan Integrated badge
...
### 6.9 Setnayan Staff Admin (`admin.setnayan.com`)
...
- ✓ 5 user roles: couple, guest, vendor, coordinator, Setnayan Staff (+ stylist as coordinator subtype)
...
- ✓ Vendor directory with category, location, budget, free-transport zone, Setnayan Recommended filters
- ✓ Vendor application + Setnayan Staff approval workflow
- ✓ Setnayan Certified flag (manual verification by staff)
- ✓ Setnayan Recommended badge (curated list)
...
- ✓ Setnayan Exclusive QR placements per package

### Vendor recruitment

- ✓ Couples can add their own vendors that aren't yet on Setnayan
- ✓ Setnayan Staff outreach pipeline (status: pending_outreach → contacted → declined / joined / certified)
...
- ◐ Custom subdomain (`maria-juan-2026.setnayan.com`) — DNS work (V1.5 if not ready)
- ✓ RSVP form integrated with `guests.rsvp_status`

### AI assistant

- ✓ Kasalan AI chat (Claude Sonnet)
...
All endpoints are Next.js App Router Route Handlers (`apps/web/app/api/.../route.ts`). All requests must include a Supabase auth session unless the endpoint is explicitly public. The admin app at `admin.setnayan.com` uses the service-role key and has its own endpoints under `/api/admin/...`.
...
- `POST /api/ai/kasalan` — Kasalan AI chat endpoint (rate-limited: 5/day free, unlimited paid)
...
### Admin (under `admin.setnayan.com/api/...`)
...
- **What it does:** powers Kasalan (couple-facing chat) and AI-driven vendor recommendation.
...
- **Critical gotchas:** strict prompt-injection defense (the AI runs against user-supplied wedding context). Always sanitize event data before passing to model. Never let Kasalan emit URLs that aren't from the Setnayan allowlist.
...
- **Critical gotchas:** create one Rekognition Collection per event (`setnayan-event-[uuid]`). Delete the collection 6 months after the wedding to comply with DPA 2012 retention.
...
- **API key setup:** Cloudflare dashboard → R2 → API tokens → create read+write token scoped to `setnayan-uploads`.
...
- **API key setup:** Resend dashboard → API keys. Verify sending domain (`setnayan.com`) via DNS.
- **Code package:** `resend`.
- **Critical gotchas:** SPF / DKIM / DMARC must be configured on `setnayan.com` for delivery to Gmail / Outlook.
...
- Vendor application + Setnayan Staff approval flow (in admin app)
...
- Vendor browse with filters (category, location, budget, free-transport, Setnayan Recommended)
...
**Sprint 3 exit criteria:** a guest scans the event QR on the wedding day, uploads a photo, and finds it auto-tagged in their personal album within 60 seconds. A couple can chat with Kasalan and get a vendor recommendation.
...
- Setnayan Staff admin tool (full feature set)
...
5. **AI recommendation:** couple uses Kasalan → gets vendor recommendation → inquires
...
- `04_App_Mockups_v1.html` — visual mockups across all roles
...
- **THIS DOCUMENT** — `07_V1_Developer_Specification.md` — build spec
...
- [ ] **Logo design:** finalize Setnayan wordmark and the per-couple monogram generator template. The monogram generator must produce a unique SVG per couple from initials + a curated set of motifs. Owner: designer. Due: Week 2 (so Sprint 1 can render real monograms on the landing page).
...
- [ ] **Subdomain strategy:** decide whether custom subdomains (`maria-juan-2026.setnayan.com`) ship in V1 or V1.5. DNS automation cost is non-trivial. Owner: tech lead. Due: end of Sprint 3.
- [ ] **PayMongo merchant account:** start KYC at Week 0 so the account is live before Sprint 4. Owner: ops. Due: Week 0.
- [ ] **Resend domain verification:** SPF / DKIM / DMARC on `setnayan.com` must be set up in Week 0 so test magic-link emails deliver to Gmail. Owner: tech lead. Due: Week 0.
