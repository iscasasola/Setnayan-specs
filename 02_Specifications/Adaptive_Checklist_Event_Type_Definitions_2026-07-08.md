...
2. **PR-2 · WIRE Layers 2 + 3 + budget (they are dead code today, not partial).** ⚠ **`checklist-taxonomy.ts` was DELETED 2026-08-06** (0 importers) — this PR CREATES it at the same path rather than editing a stub. Give `checklist-taxonomy.ts` a real reader of the event type's plan-group tree; call `resolveCategoryState` from the render path; call `computeBudgetHealth` from the checklist/budget surface. This is net-new wiring the design doc assumed already existed.
