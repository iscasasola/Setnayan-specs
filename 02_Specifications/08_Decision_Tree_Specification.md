...
> **This doc is HISTORICAL/REFERENCE.** Authoritative current state = the live site (www.setnayan.com) + shipped code + `AS_BUILT_GROUND_TRUTH_2026-06-07.md`. The deterministic flowchart logic (the 14 domains, ~100 flowcharts, calculators, lookups, PH-Catholic content) still reads true as planning *guidance*, but the surrounding product framing drifted:
> - **The AI layer is not "Kasalan."** "Kasalan AI" / "Ask Kasalan" was never the shipped name. The couple-side AI planner is **"Today's Focus" ₱1,499** (one purchase, full access). Note the couple-app Today's Focus *wizard* was further retired in code (the SKU/branding persists on the site); the "I'm stuck" button + 3-layer guidance model described here is design history, not a live surface.
> - **AI monetization changed.** "3 free Kasalan conversations, then ₱49/session or ₱199/mo" is RETIRED — there is no per-conversation AI paywall; planning AI is gated by the single Today's Focus SKU.
> - **Tiers retired.** Every "Essentials / Premium / Pro Event" tier reference (guest-count routing, budget bands, per-head matrices, the Part-5 tier enum) is superseded — those wedding tiers do not exist in V1.
> - **"Setnayan Photo Teams" (Solo/Duo/Premium) and crew tiers do not exist.** In-app capture is **Papic** (DIY seats, no Setnayan-supplied operators). The Layer-4 "Setnayan Staff" human-escalation is now the **admin console** actor.
> - **Vendor model:** any implied commission/coordinator-commission economics → **0% commission**, vendor↔customer money is OFF-PLATFORM; vendors run a free profile + a **token economy** (burn-on-answer).
>
