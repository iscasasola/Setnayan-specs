...
Single source of truth: extend `fetchWizardVendorRecommendations` to **emit the score + the top match reasons**, so every surface (onboarding find-vendor, `/vendors` marketplace, the Plan+Budget accordion, Today's Focus picks) inherits it.
