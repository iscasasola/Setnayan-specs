...
> - **The "Setnayan Concierge ₱4,999" 9-step journey / 3-day trial / DIY-vs-Concierge mode toggle (§§ 2.0a–2.0b') is RETIRED in code.** `lib/concierge.ts` ships `CONCIERGE_ENABLED = false`; the couple-app Setnayan AI *wizard* is gone and `/dashboard/[eventId]/today` redirects to event-home (RETIRED 2026-06-03 banner). The planner ships as the **"Setnayan AI" SKU at ₱1,499** (not ₱4,999). Concierge schema/enforcement columns + the `concierge_abuse_flags` machinery are dormant. Guidance now = onboarding + per-service deadline timeline (`lib/upcoming-items.ts`), not a paid assistant layer.
...
**Digital Services group in the Services tab (2026-06-03).** The Services / Add-ons surface groups the five Setnayan digital productions — **Pakanta · Animated Monogram · Pro Website · Live Venue Photo Wall · Pailaw** — under a **"Digital Services"** section (mirroring the marketplace `Design › Digital Services` child), each carrying the **✦ Setnayan** badge and its **canonical name**. This fixes the prototype's label drift — retire "Paparazzi"→Papic, "Live Stream"→Panood, "LED Background"→Pailaw, "Invitation Bundle"→Pro Website, "Custom Monogram"→Animated Monogram. Pakanta + Live Venue Photo Wall are currently **missing** from the prototype and must be added. Full canon: [Digital_Services_Cross_Surface_Map_2026-06-03.md](../03_Strategy/Digital_Services_Cross_Surface_Map_2026-06-03.md).

**Services tab is CHOICE-DRIVEN — pre-add IFF chosen (owner-locked 2026-06-03).** Every Setnayan service is **pre-added** to the Services tab (ready to avail / manage · no hunting) **only if its category was chosen in onboarding** (the screen-10 "What would you love?" picker); a service whose category was **not** chosen **does not show** here — the tab stays decluttered, the rest reachable via **"add more / browse all services."** This extends the existing "only the picked categories show" rule ([Vendor_Match_Personalization_2026-06-01.md](../03_Strategy/Vendor_Match_Personalization_2026-06-01.md) §7b · [Service_Schedule_and_Quotation_Flow_2026-06-02.md](../03_Strategy/Service_Schedule_and_Quotation_Flow_2026-06-02.md) §T1.6) from vendor categories to Setnayan's own first-party services. Category → pre-added Setnayan service(s): **Digital Services** → Pakanta · Animated Monogram · Pro Website · Live Venue Photo Wall · Pailaw · **Photo & Video** → Papic (+ SDE / Guest Stories / Thank You add-ons) · **Livestream** → Panood · **Photo Booth** → Patiktok · Pabati. *(Pakulay / Mood Board is free → always present. **Resolved 2026-06-03 (owner):** the un-categorized "direct" services — Setnayan AI · Custom QR per Guest · Indoor Blueprint · Call-Time Escalator · High Res Archive · Camera Bridge — are **always pre-added** to every account, since they have no marketplace category to be chosen through. Granularity is category-level — the 5 digital services share one "Digital Services" picker chip, so choosing it pre-adds all five.)* **Source of truth = the Setnayan first-party vendor account** (see [Digital_Services_Cross_Surface_Map_2026-06-03.md](../03_Strategy/Digital_Services_Cross_Surface_Map_2026-06-03.md) §3): the pre-added list = Setnayan vendor listings whose tile ∈ the couple's chosen tiles, plus the always-on direct services. **Embedded, not bolted-on (owner-locked 2026-06-03 · "we want it embedded on their list of services"):** these appear **inline in the couple's own list of services**, as ordinary Setnayan-vendor listings the team manages in the vendor account — **no separate admin-config surface, no new schema, no tile-normalization layer.** The existing vendor model (`is_setnayan_service` float-to-top + the listing's category tags) does the work.

**Pricing is the vendor template + shown per unit (clarified 2026-06-03).** Because each Setnayan service is a listing on the Setnayan first-party vendor account, its **cost model is the Universal Service Template** the team fills there ([Service_Specifications_2026-06-02.md](../03_Strategy/Service_Specifications_2026-06-02.md)) — **per-pax** (Starting Pax + Additional Cost/pax + `per_pax_tiers_json`, with the 2026-06-01 100-pax floor + per-50 increment encoded as tiers), **per-hour / per-event** (`price_unit` + BASIC Time coverage), **added costs** (FEES line items), and **inclusions** (INCLUSIONS / `inclusions_md`). The embedded card here **renders the price per its unit** ("₱X per event" · "₱X / hour" · "from ₱X / 100 pax"). The legacy hardcoded in-app strip (Orders · Save the Date Video · Playlist) is **retired** — the list is sourced from the Setnayan vendor listings, not a code constant. **No separate admin SKU catalog / pricing-model editor** (see DECISION_LOG 2026-06-03 "🧾 In-app services pricing IS the vendor template").

**Always-on + add-and-pay (owner-locked 2026-06-03).** Because they're Setnayan-fulfilled, in-app services are **exempt from the external-vendor engagement lifecycle** — no inquiry→accept→token→lock, no schedule date-gate, no capacity, no 7-day nudge (those would otherwise leave a Setnayan "inquiry" stuck in `requested` forever, and would wrongly date-gate an always-available service). The couple **adds and pays** through Setnayan checkout ([0034](../0034_payments_and_cart/0034_payments_and_cart.md) apply-then-pay), available regardless of the wedding date; `is_setnayan_service` governs both the float-to-top **and** this exemption. See DECISION_LOG 2026-06-03 "♾️ In-app services are ALWAYS-ON + direct-checkout".

**Supplementary, never category-exclusive (owner-locked 2026-06-03).** Adding a Setnayan service does **not** saturate its category or block an external vendor in it — a couple with Papic (Photo & Video) can still shortlist + lock a full external photographer in the same category; the two coexist. The hard-single / one-vendor-per-category saturation counter **excludes `is_setnayan_service`** — Setnayan is supplementary *"in case they need our service,"* never the category's primary vendor. See DECISION_LOG 2026-06-03 "➕ In-app services are SUPPLEMENTARY".
...
4. **Guide buckets · NEW · locked 2026-05-20.** The active home guide rendered as three tactical buckets — the truth-telling layer that surfaces to **all couples (DIY and Concierge)** because hard floors are facts, not personalized service:
   ...
   **Deadline anchor · amended 2026-06-03 (owner).** The bucket math anchors on the **earliest date the couple has chosen**, not strictly `wedding_date`. During onboarding `events.event_date` is null (the date lives as candidate-date(s) / a window), so `anchor = COALESCE(events.event_date, earliest chosen candidate/window date)` — the scheduler runs *before* a firm wedding date is locked and tightens automatically once one is set (the two formulas coincide once `event_date` exists). Two render targets: **(a) Home "Upcoming schedules"** — one chronological merge of every chosen service's deadline dates, soonest-first (the layer that succeeds the retired Setnayan AI wizard; PH statutory deadlines come from `lib/upcoming-items.ts`); **(b) per-service deadline updates** — each chosen service surfaces its own next deadline inline on the Services tab (e.g. *"Catering — finalize headcount in 12 days"*). Supersedes `computed_deadline = wedding_date - latest_by` for any event without a locked `event_date`.
