...
4. **Guide buckets · NEW · locked 2026-05-20.** The active home guide rendered as three tactical buckets — the truth-telling layer that surfaces to **all couples (DIY and Concierge)** because hard floors are facts, not personalized service:
   ...
   **Deadline anchor · amended 2026-06-03 (owner).** The bucket math anchors on the **earliest date the couple has chosen**, not strictly `wedding_date`. During onboarding `events.event_date` is null (the date lives as candidate-date(s) / a window), so `anchor = COALESCE(events.event_date, earliest chosen candidate/window date)` — the scheduler runs *before* a firm wedding date is locked and tightens automatically once one is set (the two formulas coincide once `event_date` exists). Two render targets: **(a) Home "Upcoming schedules"** — one chronological merge of every chosen service's deadline dates, soonest-first (the layer that succeeds the retired Today's Focus wizard; PH statutory deadlines come from `lib/upcoming-items.ts`); **(b) per-service deadline updates** — each chosen service surfaces its own next deadline inline on the Services tab (e.g. *"Catering — finalize headcount in 12 days"*). Supersedes `computed_deadline = wedding_date - latest_by` for any event without a locked `event_date`.
