...
- ⚠ Prefix rot: this worktree's newest local migration (20271103100000) already sorts BELOW origin/main's head (20271103100614). ⛔ **CORRECTED 2026-08-04 (this line was missed by the first correction pass — the same false belief as §1082/§1113):** ~~Migrations apply once in prefix order, so a PR-H migration allocated against this stale tree can merge with green CI and CREATE NOTHING.~~ That is **FALSE**. Deploy runs `supabase db push --include-all --yes`; `--include-all` applies out-of-order migrations (verified 13 ways). What a stale prefix ACTUALLY breaks is the **PGlite replay** (`apps/web/tests/db/replay-migrations.ts` sorts by FILENAME), so a PR-H migration depending on `20271103100614` fails every `*.db.test.ts` while prod is fine. `check-migration-timestamps.mjs` checks UNIQUE + not-hand-typed-round, **not** ordering. Allocate with `pnpm migration:new` AFTER fetching main — for replay order, not for "it won't apply" — and verify the object (SELECT to_regclass('public.<table>')) rather than schema_migrations.
