...
> 🔁 **CORRECTION 2026-09-23 · SETNAYAN AI — the ₱499 / ₱799 subscription is RETIRED, and the price varies by EVENT TYPE.** Owner, asked directly whether the intro/renewal model was still intended, verbatim: *"the one time. and each event has a different price"*.
>
> **Why this correction exists rather than a reprice row.** A sentence reading *"the full Sai is ₱499 first 28 days → ₱799 per 28 days"* was still RENDERING TO COUPLES on the Overview — mounted twice, unconditionally — while the catalogue charged a one-time amount several times that, and the renewal SKU it named (`SETNAYAN_AI_RENEW`) was `is_active=false`. It was not a price that had drifted; it was **a promise the product could not keep**. Fixed in code 2026-09-22: the copy now derives from `platform_retail_catalog_v2` and carries no number in its source.
>
> 🔑 **THIS DOCUMENT MUST POINT AT THE CATALOGUE, NOT MIRROR IT.** Mirroring is how ₱499/₱799 survived long enough to reach a couple's screen: every row that restates an amount becomes a second price list that nobody reprices. `platform_retail_catalog_v2` is admin-managed and is the only price a customer is charged (repo `CLAUDE.md`: never derive a price from a document or a code comment). **Amounts below this line are LINEAGE — read the catalogue.**
>
> ⚠ **Any row in §§ 00–8 quoting a single Setnayan AI figure is stale by construction**, including the ones that say ₱1,499 — that is now the TIER-A SIGN-UP price, not the price. Rows recording the per-USER subscription, the ₱3,999 per-event model, the ₱4,999 pass and the ₱499-entry are all superseded and kept only for lineage.
...
| Setnayan AI (`SETNAYAN_AI`) | **ONE-TIME, PER EVENT — and the amount DIFFERS BY EVENT TYPE. No figure is recorded here on purpose.** 🔁 **Owner-ruled 2026-09-23**, verbatim: *"the one time. and each event has a different price"*. The subscription model is dead — see the correction note under § 00.0 below. The charge is whichever tier row the event's type maps to (`AI_TIER_BY_EVENT_TYPE` → `AI_TIER_SKU` in `apps/web/lib/setnayan-ai-type-pricing.ts`; tier A is `SETNAYAN_AI` itself, B/C/D are price-source-only rows with `is_active=false` BY DESIGN), and a sign-up purchase pays that row's `onboarding_price_php` instead. ⛔ **Do not quote an amount from this document.** Re-measure: `select service_code, retail_price_php, onboarding_price_php, billing_period, is_active from platform_retail_catalog_v2 where service_code like 'SETNAYAN_AI%';` |
...
| Papic — shots | 🔄 **THE ROW THAT WAS HERE DESCRIBED A PRODUCT THAT NO LONGER EXISTS** — per-camera, per-day tiers (Mini ₱30 · Ltd ₱50 · Unli ₱100) with daily caps. That model was retired by the ONE-PRODUCT lock of **2026-08-11** and the row was never updated; it is replaced here, and the old text stays in the lineage banners below. **Shipped model:** one shared pot of shots per event. **Cameras are FREE and UNLIMITED.** **50 shots free on every event.** A photo spends 1 credit; a clip spends 2–8 by length (1–2s=2 · 3s=3 · 4–6s=5 · 7–10s=8). **LADDER — a SCROLLABLE list of rungs priced against ₱1 = 1 credit, plus a deeper sign-up-only discount on the same rungs.** ⚠ **NEVER re-type the figures — every prior version of this row went stale within days, including the ₱11,200/50,000 figure that stood here, and prod has since added a 100,000-credit rung no version of this table ever listed.** `platform_retail_catalog_v2` is the ONLY source (`retail_price_php` = regular, `onboarding_price_php` = sign-up price, both by `service_code` — the `PAPIC_GUEST*` rows); `papic_pass_tiers` holds the shot counts; `apps/web/tests/db/papic-ladder.expected.ts` is the pinned expectation both db guards compare against. **Mechanics that don't drift with the numbers:** `apps/web/lib/papic-anchor-ladder.ts` — the owner types a handful of anchor prices, every other rung derives from the anchor rate in force at that credit count, and the admin screen writes the derived prices straight into the catalog (the module's own fallback constant is not the price; the catalog is). ⚖ **A rung can be deliberately absent** — the owner has removed one before, when a lower rung priced the same as a higher one made it irrational to pick; check the live catalog, don't assume any prior list is complete. 🚨 **A rung is THREE places**: the catalog row, the tier row, AND a line in `lib/sku-activation.ts` — a sellable rung missing from that map takes the money and grants **zero** shots, silently. |
| Papic add-ons | ⚠ **RE-MEASURED AGAINST THE LIVE CATALOG 2026-08-26 — the row that was here said all five were ACTIVE + PAID and three of those claims were false.** Live: **Thank You ₱2,500 ACTIVE** (owner 2026-08-27 · was ₱2,499 — the only paid Papic add-on left) · **Stories RETIRED / off sale 2026-08-11** (the ₱2,000 row sold nothing — the story maker has no entitlement gate) · **Pabati RETIRED FROM THE PRODUCT 2026-08-23** (owner: *"we do not need pabati. retire it because it is part of papic"*; the capability survives as an ordinary clip challenge) · **Kwento FREE since 2026-08-21** (owner) · Patiktok ₱1,500/day. 🔑 The 2026-07-17 recut this row called *"spec-only, unresolved"* has since RESOLVED IN CODE, in the owner's own direction — which is why the row was wrong: it recorded a pending decision and nobody came back when the decision landed. **Read the catalog, never this row.** |
| **Event Hub Pro** (`COUPLE_WEBSITE_PRO`) | 💰 **REPRICED 2026-09-25 (owner, verbatim): "make it 5000 with 40% off becoming 3000 on onboarding."** Regular **₱5,000**, sign-up (onboarding) **₱3,000** (40% off — the same 40% carried forward from the same-day ₱3,500/₱2,100 pair below, onto these new numbers). `retail_price_php` = 5000, `onboarding_price_php` = 3000 on `platform_retail_catalog_v2`. Builder branch `rd/event-hub-pro-5000`, migration `20271247699024`. See `DECISION_LOG.md` 2026-09-25 and `research/EVENT_HUB_PRO_PRICE_BENCHMARK_2026-09-25.md`. Read the catalog, never this row. _(Superseded pricing history below, kept for lineage.)_ 🏷 **RENAMED 2026-08-27 (owner): the catalog title is now "Event Hub Pro", was "Couple Website PRO" — price and `service_code` UNCHANGED** (migration `20271171000513`). The corpus and several rendered surfaces had said Event Hub Pro for weeks; the catalog row was the last place carrying the old name, and it is the one an admin and a receipt read. Was ₱3,500 (same-day predecessor ₱2,100 sign-up), previously described as **REACTIVATED** (owner 2026-07-22 · was ₱4,999, deactivated) — the umbrella and the ONLY way to buy the now-bundle-only **Cinematic Reveal + Editorial PRO**; confers both via `SKU_OWNERSHIP_ALIASES` + drops the watermark. ✅ IN CODE PR **#3564** (migration `20270915796315`). (Supersedes the prior DEACTIVATED state + the "Website Upgrade ₱3,500 spec-only" note.) |

💸 **THE SIGN-UP DISCOUNT — owner-ruled 2026-08-28, and it is now a RULE, not one SKU's quirk.** Owner: *"we give them a 10% discount if they purchase now. They can order later, but they will lose the 10% discount."* · *"10% for all purchase on onboarding"*. **Everything bought DURING the create flow is at least 10% off; order it afterwards and you pay retail.** The mechanic is `platform_retail_catalog_v2.onboarding_price_php`, which already existed and had already been charged for Setnayan AI since it was built — the sixteen `PAPIC_GUEST%` rungs simply never carried one, so the create flow quoted the *later* price at the one moment the earlier one applied (migration `20271176315255`, PR [#4943](https://github.com/iscasasola/setnayan-platform/pull/4943)). 🔴 **TAKEN LITERALLY IT WOULD HAVE RAISED THREE PRICES:** every Setnayan AI tier already discounts **40–50%** at sign-up (`SETNAYAN_AI` ₱2,499→₱1,499 · `_B` ₱1,499→₱899 · `_C` ₱899→₱499 · `_D` ₱199→₱99), so assigning 90% of retail would have moved the flagship sign-up price to **₱2,249 — a ₱750 RISE delivered by a discount ruling.** ⇒ **10% is written as a FLOOR (`LEAST`), never as an assignment:** at least a tenth off, and a row already carrying a deeper sign-up price keeps it. The migration refuses to commit if any tier ends up higher, if an active rung is left undiscounted, or if any price lands on a fraction of a peso. ⚠ **No amount is re-typed anywhere** — the figure is derived from `retail_price_php`, so a reprice needs no second edit. ⛔ `SETNAYAN_AI_RENEW` is out of scope: nobody renews during onboarding, and it is the one row where 10% lands on ₱719.10. 👁 **It must also be READABLE** (owner, same day: *"Onboarding discount should be visible and easy to understand that this discount only applies on onboarding"*) — stated in three places (a banner before choosing, each item's own saving with its later price, and the total), every one of which disappears when there is nothing to save. **No invented "was" price:** a rung with no discount lists at what it costs.
...
**VENDOR side — active catalog (`vendor_billing_catalog`):** Free (Verified) ₱0 · Solo **₱1,000/28d · ₱10,400/yr** · Pro **₱2,500/28d · ₱26,000/yr** · Enterprise **₱10,000/28d · ₱104,000/yr** (🔁 **owner price sheet 2026-08-27 · migration `20271171000513`** — Enterprise 28d ₱8,000 → ₱10,000; annuals Solo ₱10,000 → ₱10,400 · Pro ₱25,000 → ₱26,000 · Enterprise ₱80,000 → ₱104,000. **Every annual is exactly `28-day × 10.4`** — thirteen periods with 20% off — recorded as an observation, never encoded as a rule) (🔁 **round-number reprice shipped 2026-07-22 · PR #3513, merged** — off the charm ₱999/₱2,499/₱7,999 · ₱9,999/₱24,999/₱79,999; annual = 10× the 28-day fee — ⚠ **that multiplier is SUPERSEDED: annual is 10.4× since 2026-08-27**, so the annual discount is **20%**, not the ~23% the older rows below quote) · **Custom base ₱11,000/28d** (owner 2026-08-27 · was ₱8,999) + add-ons UNCHANGED (Reach +100 km ₱499 · Nationwide ₱2,499 · +1 event slot ₱499 · +100 photos ₱99 · custom domain ₱499) — **Custom remains the unlimited tier ABOVE Enterprise.** 🚨 **THE SAME DAY'S ENTERPRISE RAISE INVERTED THE LADDER AND THE BASE WAS RAISED TO UN-INVERT IT:** Enterprise went to ₱10,000 while Custom's base sat at ₱8,999, so the tier above cost ₱1,001 LESS than the tier below. Caught before anyone was quoted it (two prod vendor profiles, both `solo`). ⚠ **It had been predicted in writing and the prediction changed nothing** — `Vendor_Subscription_Ladder_2026-07-22.md` had carried the warning for five weeks. ✅ **The rule is now a build guard, not a note:** `apps/web/tests/db/custom-sits-above-enterprise.db.test.ts` reads BOTH prices out of the catalog — neither is typed into it — and fails if Custom's base ever falls to or below Enterprise's 28-day price. It pins the RELATIONSHIP, never the amounts. 🔑 **The base is genuinely the floor** — checked in the quote math, not assumed: `computeCustomQuote` floors at base AND the per-org admin discount is floored at base too, so no composition and no discount can quote a Custom plan under Enterprise. 🔴 **OPEN ₱500 QUESTION:** the signed rate card's own construction is *Enterprise + ₱1,500 white-glove premium* (owner-decided 2026-07-04 — how the ₱8,999 was derived), which would give **₱11,500**, not ₱11,000. Recorded rather than silently corrected; the number is the owner's. 🔓 **AND A RETIREMENT WAS RULED AND REVERSED THE SAME DAY:** the tier was briefly ruled retired, then put back on sale once it emerged that **an `is_active` flip is not a retirement when a code-side fallback exists** — `lib/vendor-custom-catalog.ts` substitutes a hardcoded literal for any missing row, so the flag would have hidden Custom from `/vendors` while the configurator kept quoting and selling it. Nothing was applied; all six rows stay active. 🔑 **AND `SETNAYAN_AI_RENEWAL_FALLBACK_PHP = 799` WAS INVESTIGATED AND LEFT ALONE — STALE MODEL, NOT A STALE NUMBER.** Read out of prod: `SETNAYAN_AI_RENEW` really is ₱799 with `is_active=false`, so the constant matches its row exactly. What is dead is the intro/renewal MODEL (retired 2026-07-10, superseded by the per-event-type ladder and then the sign-up/regular split). Its only consumer has **zero non-test callers**, both live mint paths go through `resolveOrderChargeCentavos`, and `events.setnayan_ai_intro_used` has **no writer**, so even a revived resolver would return the INTRO tier — **nobody can be charged ₱799 through any screen**. Setnayan AI is one-time per event (owner-locked 2026-07-26). The honest fix is deleting lineage-only code, which is its own change. ✅ Prod also settled a contradiction source could not: `platform_settings.setnayan_ai_per_event_pricing_enabled` is **TRUE**. 🔴 **AND A REAL RISK FOUND IN PASSING, REPORTED NOT FIXED:** that module degrades a Supabase read ERROR into "row absent" (`rows ?? []`) and then charges the hardcoded fallback instead of refusing — the opposite of SEC-7's *"no server-resolvable price ⇒ REFUSE THE SALE"*. Inert in the dead renewal path, but the **LIVE** `resolveSetnayanAiTypePricePhp` shares the shape, on the one product that has actually sold. 🏷 **AND THE THREE ANNUAL ROW TITLES WERE RETITLED IN THE SAME MIGRATION, because they stated the OLD discount as a promise:** *Solo … (Annual · save 12 weeks)* and *Pro / Enterprise … (Annual · save ~23%)* all now read **save 20%**. At 10.4× the saving is exactly 20% on all three (13,000 vs 10,400 · 32,500 vs 26,000 · 130,000 vs 104,000), so the figure is precise and needs no "~". Shipping the new prices under the old names would have advertised a discount we no longer give. The same claim was hardcoded in two rendered places — the annual badge on the subscription cards and the cycle-toggle hint — and both moved with it; the per-tier peso saving beside them is COMPUTED and re-derives on its own. ⚠ **Custom's annual is still 10× (~23%)** — the owner did not rule on it, so the three tiers and Custom now discount annual differently. Flagged, not changed. · Additional Branch **₱1,000/28d** (owner 2026-08-27 · was ₱999) · Extra Team Seat ₱250/28d · **token packs flat ₱200/token** (5/10/25/50/100 = ₱1,000 / ₱2,000 / ₱5,000 / ₱10,000 / ₱20,000 · `vendor_token_pack_4` retired · PR #3138 / migration `20270728100000`) · burn = flat **1 token (₱200)** per inquiry answer · **0% commission**. ⚠ Two corrections vs the prior § 00 vendor line: **(1) NO 100-free-tokens-on-verify** — retired 2026-06-17 (migration `20270110320020`); tokens come only from admin grants · subscription bundles · paid packs (the "(100 free on verify)" text previously here was stale). **(2) Vendor subdomain (`vendor_subdomain` ₱999/yr) — dual state:** seeded ACTIVE by migration `20270712300100`, but the LIVE row is `is_active=false` (admin-deactivated pending provisioning) — recorded as-is, no winner guessed; the couple-side `EVENT_SUBDOMAIN` is the active one. Boosted Ads / Sponsored Boost stay HELD (mechanism unbuilt).
...
Setnayan AI **₱1,499 one-time, per event** (✅ SHIPPED + ON PROD — migration `20270729889508`; single SKU · owner FINAL 2026-07-12 · no monthly/annual/₱4,999-pass/₱499-entry/tiers) · **Event Hub Pro ₱5,000** (repriced 2026-09-25, owner: "make it 5000 with 40% off becoming 3000 on onboarding" · supersedes the same-day ₱3,500 · title renamed from "Couple Website PRO" by owner ruling 2026-08-27 · the umbrella that unlocks the now-bundle-only Cinematic Reveal + Editorial PRO) · Editorial PRO **bundle-only (via Website PRO)** · Cinematic Reveal **bundle-only (via Website PRO)** · Animated Monogram / **Monogram PRO ₱1,000** (owner 2026-07-22 · incl. Live Background) · Pakanta **₱2,500** · 3D Plan **₱1,500** (owner 2026-07-23 · host-activation, one-time/event · interim ₱1,000 + ₱2,999 both RETIRED · ✅ **SHIPPED to the live catalog 2026-08-02** (migration `20271032178949`, PR #4033 — prod object-verified at ₱1,500, `one_time`, not pax-priced)) · Custom Subdomain **₱999/yr** · Keep Full-Res **RETIRED in code** (PR #3523 · Preserve = 6-month, folded into Papic) · **Live Studio ₱3,000/event** (owner 2026-08-27 · was ₱2,999) — the unified SKU (owner 2026-07-25; Cast+Roam merged), ✅ in code + `is_active=TRUE` but **flag-dark and name-excluded from the customer catalog**, so not sellable yet; **both per-day Cast SKUs are RETIRED** (Desktop `PANOOD_SYSTEM` ₱2,500/day by migration `20271005180040` / PR #3716 — it closed a ₱500 alias arbitrage; Mobile ₱1,500/day 2026-07-21) · single-cam livestream still FREE · YouTube via the couple’s own OBS (Wave 9 removed the couple’s YouTube-ACCOUNT requirement, not the encoder), Facebook Live supported via OBS with a mandatory 30-day-replay-deletion warning · Live Background **bundle-only (via Monogram PRO)** · Live Photo Wall **FREE** (owner 2026-08-11 · was ₱2,500) · Camera Bridge **₱500** (event-wide) · Papic **Mini ₱30 · Ltd ₱50 · Unli ₱100 per cam·day** — capture points 20/20/70/∞ (1 photo = 1 pt · 5-s clip = 3 pts) shipped in `papic_tier_config`; **free cameras + caps are DUAL STATE** (app still ships 5 free + legacy caps Ltd ₱9,000 / Unli ₱15,000; the v3 3-free + Mini ₱6,000 / Ltd ₱10,000 / Unli ₱15,000 wedding-only caps are shipped-but-unread — both collapse when Papic v3 PR-3 + free-tier enforcement land as ONE atomic PR, owner 2026-07-19) · Papic add-ons ALL PAID in code: Patiktok **₱1,500/day** (owner 2026-07-22 · was ₱1,499) · Thank You **₱2,500** (owner 2026-08-27 · was ₱2,499 — the corpus § 00.H row had already been written at ₱2,500; the catalog now agrees) · Kwento **₱299** · Pabati **₱1,299** · Stories **RETIRED / OFF SALE 2026-08-11** (owner — migration `20271132214645`; the ₱2,000 row sold **nothing**: the story maker has no entitlement gate and no code reads whether it was bought. This lands the "Stories FREE" half of the 2026-07-17 recut **in code**, by retiring the paid row rather than repricing it; ⚠ the Kwento/Pabati halves of that recut remain **spec-only, not in code — still the Papic-pricing session's**) · Custom QR **FREE** · Music Creator **RETIRED** (folds into Pakanta). **No à-la-carte bundles beyond Website PRO** — Essentials/Complete removed 2026-06-29; **Event Hub Pro ₱5,000 is the sole umbrella** (reactivated 2026-07-22 · repriced 2026-09-25 · § 00.G #1–#3). SRP = live sum of the à-la-carte SKUs above. (Retired & inactive: RSVP / RSVP Pro / Event Website / Editorial Website standalones · Papic Guest / Papic 5 Seats · SDE · Auto-Recap · Unlock-all-Papic (no live row).)
...
> **🚫 ALL BUNDLES REMOVED 2026-06-29 (owner: "no more essentials and complete").** Both bundle tiers are cut — `GUIDED_PACK` ("Setnayan Essentials" ₱12,999) + `MEDIA_PACK` ("Setnayan Complete" ₱27,999) deactivated in `platform_package_catalog`. (The older "Guided Planner Suite ₱11,999 / Comprehensive Media Pack ₱16,999" names were already stale aliases of those same codes.) **The model has NO bundles:** Free → Setnayan AI (₱1,499 one-time — see § 00) → à-la-carte SKUs. ~~`PAPIC_UNLOCK` ₱15,000/day is an à-la-carte Papic-day SKU, NOT a tier bundle — it stays active.~~ 🔴 **BOTH PAPIC UNLOCK BUNDLES RETIRED 2026-08-27 (owner)** — `PAPIC_UNLOCK` ₱15,000 and `PAPIC_UNLOCK_LTD` ₱9,000 are `is_active=false` (migration `20271171000513`). Superseded by the 16-rung credit ladder: with 50,000 credits at ₱11,200, a ₱15,000 "unlock everything" package no longer prices sensibly beside it. **Nothing was stranded** — measured before writing: production holds two orders in its entire life and neither is a bundle, and `event_software_activations_v2` holds zero rows for either code. `is_active` is honoured by every layer that renders or charges a bundle, checked rather than assumed. So `platform_package_catalog` now has **no active row at all**.
