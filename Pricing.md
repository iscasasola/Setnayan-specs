...
| Editorial PRO (`EDITORIAL_PRO`) | **BUNDLE-ONLY** (owner 2026-07-22) — `is_active=false`; obtained ONLY via Website PRO ₱3,500 (`SKU_OWNERSHIP_ALIASES`). Was standalone ₱2,999; the `editorial-pro` card is retired. Existing owners keep it. ✅ IN CODE PR **#3564**. ⏳ needs `supabase db push`. |
| 3D Plan (`SEATING_3D`) | **₱1,500** — host-activation, one-time per event (owner 2026-07-23 · was interim ₱1,000 · was ₱2,999). ⚠ **NOT yet in code:** live catalog is still ₱2,999 (`20270710619774`); repo/DB migration owed. **Value** = an interactive "gaming-like" 3D seat plan: view own seats + vendors + **co-presence (see other accounts roaming the room = shared-room slice 8, built flag-off `NEXT_PUBLIC_PLAN3D_SHARED_ROOM`)**. **RETIRE everything else (owner):** the ₱2,999 standalone, the interim ₱1,000, AND the #3526 vendor-enabled couple discount are all retired → ₱1,500 is the single host price. The **₱1,500/28d vendor 3D-Booth add-on** (#3526 · `vendor_3d_booth` · § 00.G #8) is SEPARATE + unchanged (recurring vendor ad/booth presence, not a couple discount — same number, different cadence + payer). |
| Custom Subdomain (`EVENT_SUBDOMAIN`) | ₱999 / year · active — remaining gap is provisioning (wildcard DNS + routing), not billing |
| Keep Full-Res (`HIGH_RES_ARCHIVE`) | **RETIRED IN CODE 2026-07-22 (PR #3523)** — `is_active=false` via migration `20270908796702`, so the studio buy card auto-hides (it renders only when the price is truthy, gated on `is_active`) and it drops from the `is_active`-filtered catalog readers. (History: revived ACTIVE by `20270723385655` · owner 2026-07-11; the 2026-07-17 retirement is now landed in code — the prior "spec-only, not in code — unresolved" contradiction is RESOLVED.) Legacy owners keep access — `ownsKeepFullRes` + the full-res drop sweep gate on ORDER OWNERSHIP, not the catalog flag. |
...
| ~~Live Studio — Desktop (`PANOOD_SYSTEM`)~~ · ~~Mobile (`PANOOD_SYSTEM_MOBILE`)~~ | ~~₱2,500 / day~~ · ~~₱1,500 / day~~ → **🚫 BOTH RETIRED** (`is_active=FALSE`). Mobile retired 2026-07-21; **Desktop retired 2026-07-26 by migration `20271005180040`** (PR **#3716**) — because Wave 6 made `PANOOD_SYSTEM` an ownership ALIAS for `LIVE_STUDIO`, so a ₱2,500 Cast purchase would have conferred the ₱2,999 unified controller: a **₱500 arbitrage on a live buy button**. Checkout already refuses a retired SKU (`resolveServiceSellability` → `retired`, ahead of both charge resolvers, pinned by an index-ordering test), so the money was never at risk; the buy CTA is hidden while the **launch** CTA stays (gating both would strand an existing buyer). **Zero orders exist for any Live Studio SKU** (verified by SQL at cutover, § 4i), so the retirement stranded nobody. Row preserved + reversible; the alias still resolves historical orders + comp grants (ownership reads `orders`/`comp_grants`, never the catalog — design-pinned). _(Lineage: ₱3,499/day single tier → split Mobile/Desktop 2026-07-08 → repriced ₱1,500/₱2,500 2026-07-20 PR #3425 → both retired into `LIVE_STUDIO`. The "do NOT charm-correct back to ₱1,499/₱2,499" note is moot — neither row is sellable.)_ |
...
| Live Photo Wall (`LIVE_WALL`) | ₱2,500 (catalog row bills `one_time`; the "/day" framing is display copy) |
...
| ~~Essentials ₱12,999~~ · ~~Complete ₱27,999~~ | — | **🚫 BUNDLE TIERS REMOVED 2026-06-29 (owner: "no more essentials and complete").** `GUIDED_PACK` + `MEDIA_PACK` deactivated (live as of PR #2409). The ladder is now **Free → Setnayan AI (₱1,499 one-time — see § 00) → à-la-carte** (no bundles). |
...
> **(code-sync 2026-07-19):** in the live catalog `COUPLE_WEBSITE_PRO` is **deactivated** (no new sales · `20270712300000`); `EDITORIAL_PRO` ₱2,999 + `STD_PREMIUM_OPENINGS` ₱999 sell standalone; and `SEATING_3D` is **ACTIVE** at ₱2,999 (the "inactive until the gate is wired" note below is stale — activated `20270710619774`). The unlock rows below are otherwise lineage.
...
| ~~Unlock all of Papic~~ (`PAPIC_UNLOCK`) | — | — | **RETIRED 2026-07-17 (owner).** Built for 2 tiers, unworkable with 4 (Mini/Ltd/Unli + Lite) and already an accidental ~₱3,000 markdown vs à-la-carte. The per-tier caps do the "won't overpay" job; add-ons bill à-la-carte. Deactivate the SKU. | — |
...
| Live Photo Wall (`LIVE_WALL`) | ~~₱2,500/day~~ | — | **HIDDEN — do not surface yet 2026-07-17 (owner** "hide and do not use this addon yet"**).** SKU exists but must NOT appear in setup or catalog. _(When revived: a strong Papic Lite attach — a crowd venue-screen collage.)_ | [0012_papic.md](0012_papic/0012_papic.md) |
...
| ~~Keep Full-Res~~ (`HIGH_RES_ARCHIVE`) | — | — | **RETIRED ENTIRELY 2026-07-17 (owner).** No paid full-res hosting at all. Full-res = the couple's Google Drive (free) OR download within the **6-month window**; after that only the compressed gallery remains (kept forever). ⚠ A couple who neither Drive-syncs nor downloads within 6 months **loses originals from our side** — surface this clearly in the gallery. **Cascade:** removes the deep-cold / B2 tier for full-res entirely — only the compressed 5-yr gallery uses cold storage now. Also retires the "Preserve — pick/select-all" step. | — |
