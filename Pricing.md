...
**VENDOR side — active catalog (`vendor_billing_catalog`):** Free (Verified) ₱0 · Solo **₱1,000/28d · ₱10,400/yr** · Pro **₱2,500/28d · ₱26,000/yr** · Enterprise **₱10,000/28d · ₱104,000/yr** (🔁 **owner price sheet 2026-08-27 · migration `20271171000513`** — Enterprise 28d ₱8,000 → ₱10,000; annuals Solo ₱10,000 → ₱10,400 · Pro ₱25,000 → ₱26,000 · Enterprise ₱80,000 → ₱104,000. **Every annual is exactly `28-day × 10.4`** — thirteen periods with 20% off — recorded as an observation, never encoded as a rule) (🔁 **round-number reprice shipped 2026-07-22 · PR #3513, merged** — off the charm ₱999/₱2,499/₱7,999 · ₱9,999/₱24,999/₱79,999; annual = 10× the 28-day fee) · 🔴 **CUSTOM TIER RETIRED 2026-08-27 (owner) — ENTERPRISE IS NOW THE TOP PURCHASABLE TIER.** All six rows (~~base ₱8,999 · Nationwide ₱2,499 · Reach +100 km ₱499 · +1 event slot ₱499 · custom domain ₱499 · +100 photos ₱99~~) are `is_active=false` via migration `20271171000513`; anyone above Enterprise's caps is handled by hand, off-platform. 🔒 `vendor_tier_rank()` and the `vendor_tier_state` enum are deliberately UNTOUCHED — retiring what can be BOUGHT was the ruling, deleting the tier is not; production held **zero** vendors on custom (two profiles, both `solo`). 🚨 **BUT THE FLAG DOES NOT CLOSE THE DOOR, and this is unfinished:** `lib/vendor-custom-catalog.ts` substitutes a hardcoded literal for any of these rows that goes missing — its own docblock says deactivating a row *"is not a retirement"* — and the vendor-side configurator is still linked from the subscription page, still quoting the same numbers. Closing it for real means deleting the axes from `CUSTOM_SKU_CODES` + `CUSTOM_UNIT_PRICE_FALLBACK`. What DID change customer-side: `/vendors` and the homepage read through `fetchV2VendorCatalog`, which filters `is_active`, and `customFrom` has no peso fallback — so the public "from ₱8,999" figure is gone. · Additional Branch **₱1,000/28d** (owner 2026-08-27 · was ₱999) · Extra Team Seat ₱250/28d · **token packs flat ₱200/token** (5/10/25/50/100 = ₱1,000 / ₱2,000 / ₱5,000 / ₱10,000 / ₱20,000 · `vendor_token_pack_4` retired · PR #3138 / migration `20270728100000`) · burn = flat **1 token (₱200)** per inquiry answer · **0% commission**. ⚠ Two corrections vs the prior § 00 vendor line: **(1) NO 100-free-tokens-on-verify** — retired 2026-06-17 (migration `20270110320020`); tokens come only from admin grants · subscription bundles · paid packs (the "(100 free on verify)" text previously here was stale). **(2) Vendor subdomain (`vendor_subdomain` ₱999/yr) — dual state:** seeded ACTIVE by migration `20270712300100`, but the LIVE row is `is_active=false` (admin-deactivated pending provisioning) — recorded as-is, no winner guessed; the couple-side `EVENT_SUBDOMAIN` is the active one. Boosted Ads / Sponsored Boost stay HELD (mechanism unbuilt).
