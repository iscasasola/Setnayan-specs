...
| #19f | **Generate crypto secrets** — `ENCRYPTION_KEY` (AES-256-GCM, used by `lib/encryption.ts` PR #152) · `CRON_SECRET` · `OAUTH_REFRESH_CRON_SECRET` · `INTERNAL_WORKER_SECRET`. Without these, OAuth flows fail to decrypt + admin/cron endpoints return 401. Generate each with `openssl rand -base64 32`, paste into Vercel env. | Same day (5 min) | Free | 🟡 (pre-generated · awaiting Vercel env paste — refresh 2026-05-22) | (new — added 2026-05-20) |
