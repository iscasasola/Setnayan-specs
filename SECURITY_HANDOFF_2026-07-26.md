...
- **Price display mismatch:** `/studio/setnayan-ai` renders the per-event-type price **ungated** while checkout's per-type branch is flag-gated — a `date` event is shown ₱99 but charged ₱1,499. Checkout re-resolves server-side so nothing under-charges, but you are **displaying less than you take**. Settle before flipping the flag.
