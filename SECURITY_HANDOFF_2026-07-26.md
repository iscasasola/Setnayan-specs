...
- **CSP `script-src`** — 🟡 **MEASUREMENT SHIPPED 2026-07-30, PR [#3921](https://github.com/iscasasola/setnayan-platform/pull/3921); enforcement pending owner review.** The deferral in `next.config.ts` is right that a strict policy must enumerate every external origin — **and that list is a GUESS until measured**, which is why this ships `Content-Security-Policy-Report-Only` + `/api/csp-report` rather than an enforced policy. The enforced header is still `frame-ancestors`-only, and a test pins it that way: **a same-PR flip to enforcement is the failure mode.** ⚠ `'unsafe-inline'`/`'unsafe-eval'` stay in `script-src` deliberately — a strict script-src fires on Next's own inline hydration bootstrap every page load and drowns the origin signal; removing inline needs a nonce in middleware and its own measurement. `/keynote` + `/proto` excluded (47 files of inline Babel would bury the signal; already robots-disallowed). 🔑 **Reports are MINIMISED to directive + blocked ORIGIN in a pure module** — a raw CSP report carries `document-uri`/`referrer`/full `blocked-uri`, i.e. event slugs, guest tokens and signed R2 URLs, so an unminimised sink would violate 0035's *no PII in logs* rule: **a security control must not become the thing that leaks.** ⏭ **OWNER: let it run, read the reports, then enforce in its OWN PR.** Violations appear as `[csp-report-only] <directive> <origin>` in Vercel logs / Sentry.
