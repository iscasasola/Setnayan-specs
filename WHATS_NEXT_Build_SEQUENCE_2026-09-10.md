...
| N4 part 4 render credits couple-only | ✅ MERGED | #5431 |
...
| LOCK-PATH CAPACITY (Opus · high) — was "H6 follow-up" | 🔨 in the N5 session (B2 + N4 landed) | BUG (measured by S2 in prod `pg_policies`, 2026-09-11): the couple-side per-card daily-limit gate (`vendors/actions.ts` #2, `soft_hold_limit_reached`) counts other couples' bookings under the couple's OWN session, and RLS lets a couple see only their own events → it always counts 0 and never refuses: a supplier's per-card daily limit does not stop double-booking. Fix: ONE `SECURITY DEFINER` count function used by the #2 gate AND by H6's helper (remove H6's tripwire in the same PR); make both day-precision-only (a month-precision event stored on the 1st must not count as a booking on the 1st). Time slots and calendar blocks are already enforced by definer functions. Prod today: 0 cards with a daily limit, 0 confirmed bookings with a service — nothing broken yet. ⚠ S2's handover notes for whoever edits the H6 files (2026-09-11): (1) ORDER — the #2 gate must REALLY refuse (definer count) in the SAME PR that teaches the mirror daily_capacity and removes the tripwire; never the mirror first. (2) The differential only calls SQL; if the daily_capacity check becomes a SQL function, the differential must call it too, else add seeded cases (at cap vs under, confirmed statuses, archived rows, date precision) and keep the not-vacuous bound (refusals ≥ 20 and ≤ cells − 20) meaningful. (3) lib/h6-mirrors-the-booking-path.test.ts: `auditMirror` flags `\bdaily_capacity\b` and its MUTATION test asserts that flag — replace both with a real predicate pair (gate SQL ↔ mirror CTE) and add the new CTE to `mirrorParts`' CTE list. (4) A CREATE OR REPLACE keeps grants; a signature change (DROP+CREATE) must re-state REVOKE ALL FROM PUBLIC, anon, authenticated + GRANT EXECUTE TO service_role; exposure baseline unchanged vs main. |
