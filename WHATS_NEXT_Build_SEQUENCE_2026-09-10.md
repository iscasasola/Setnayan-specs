...
| FOLLOW-UP · data export gap | 🔨 OWNED BY S2 | Found by S2 (H4): the couple's payment ledger (event_vendor_payments) is event-tier and NOT included in the personal data export (RA 10173 right of access) — predates H4. Check what the export's roster says and add it, or record why not. |
| FOLLOW-UP · a re-sent deposit erases its refusal | 🔨 OWNED BY S2 (FOLLOW-UPS A) | Found by S2 (H4): `guard_event_vendor_deposit_ack` lets a session clear deposit_declined_* / deposit_dispute_*, and `recordDeposit` (vendors/actions.ts ~4491) does so when a couple re-records — so a refused deposit leaves the admin queue with no trace. Existing, deliberate re-send design; the fix is to keep the earlier refusal as history (audit row) while still allowing the re-send. Installments can't do this (a re-send is a new row). |
