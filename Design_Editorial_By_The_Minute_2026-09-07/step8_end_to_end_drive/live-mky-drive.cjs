// STEP 8 · "Make it yours" driven against the LIVE site (www.setnayan.com) as the signed-in testnayan1
// host, on "Song Desk Test Night" (10 captures) — steps 4 + 6's drive (step4_make_it_yours_drive/),
// re-pointed from the local stand-in page at the real Story Maker: the real page, the real read, the
// real save (save_story_arrangement) and the real database. Only the fixture captures' image BYTES are
// drawn stand-ins (their R2 objects were never uploaded) — see e2e-lib.cjs.
//   PART=4 → the photo half · PART=6 → words, toolbar, moments, sets (each needs a fresh arrangement:
//   the drive cannot reset the live database, so the session resets it between parts).
...
if (!['4', '6', 'gaps'].includes(PART)) throw new Error('PART=4, 6 or gaps — each needs a fresh arrangement, reset by the session between parts');
const OUT = L.OUT;
const TOTAL = 10;
...
async function facts(page) {
  return page.evaluate((ROOT) => {
    ...
    const hint = document.querySelector('[role="status"][aria-live="polite"][class*="make-it-yours_hint"]');
    return {
      pills,
      films,
      onPage,
...
/** The live Story Maker, on "The story" step (the rail opens on "The desk" after every load). */
...
// `?reset` cannot reach the live database — the session resets the arrangement BEFORE each part.
...
/**
 * LIVE ONLY: the real dashboard has a fixed header and, on a phone, a fixed bottom menu. "In the
 * viewport" is not "under the finger" — a photo in the bottom 100px sits under the menu, and a tap
 * there opens the menu's link (measured: the phone drive's double tap landed on "Overview" and left
 * the page). A person scrolls it clear first; so does the drive, before every coordinate press.
 */
async function clearOfChrome(loc) {
  await loc.scrollIntoViewIfNeeded().catch(() => {});
  await loc.evaluate((e) => {
    const r = e.getBoundingClientRect();
    if (r.top < 80 || r.bottom > innerHeight - 100) e.scrollIntoView({ block: 'center' });
  }).catch(() => {});
}
const center = async (loc) => {
  await clearOfChrome(loc);
...
async function run(phone) {
  ...
  const { ctx, log } = await L.hostContext({ phone, dark: !!process.env.DARK });
  const browser = { close: () => ctx.close() };
  const page = ctx.pages()[0] || (await ctx.newPage());
  // What the page SENT on its last save — the live stand-in for the harness's `[data-harness-doc]`.
  page.on('request', (r) => {
    if (r.method() !== 'POST' || !r.headers()['next-action']) return;
    const body = r.postData() || '';
    if (body.includes('"moments"')) page.__lastSave = body;
  });
  if (process.env.TRACE) {
    page.on('response', async (r) => {
      if (r.request().method() !== 'POST' || !r.url().includes('/story')) return;
  ...
  const hr = await page.evaluate(() => { const r = document.querySelector('[role="status"][aria-live="polite"][class*="make-it-yours_hint"]').getBoundingClientRect(); return { top: r.top, bottom: r.bottom, left: r.left, right: r.right, vh: innerHeight, vw: innerWidth }; });
  ...
  if (phone) {
    ...
    await page.evaluate(() => {
      const h = document.querySelector('[role="status"][aria-live="polite"][class*="make-it-yours_hint"]');
  ...
  await page.screenshot({ path: `${OUT}/drive-${phone ? 'phone' : 'desk'}${process.env.DARK ? '-dark' : ''}-tray.png` });
  {
  ...
  // ── WHAT COULD NOT BE READ · NO RUN OF SHOW ─────────────────────────────────────────────
  // Harness-only states: an unreadable pool cannot be produced on the live database, and this event
  // HAS a run of show. Both are proven by the step-4/6 drive over the stand-in; the no-schedule state
  // is driven live on the new celebration in live-e2e (before its schedule is added).
  console.log(`SKIP [${tag}] unreadable pool + no run of show — harness-only on this event (see header)`);
  } // end PART 4

  // ── STEP 6: WORDS, THE TOOLBAR, MOMENTS, NAMING, SETS, REORDER ─────────────────────────
  if (PART === 'gaps') await require('./live-10a-gaps.cjs').gaps({ page, ctx, phone, tag, check, facts, whole, waitSaved, open, touchDrag, mouseDrag, center, ROOT, TOTAL, OUT, log });
  if (PART === '6') await require('./live-mky-step6.cjs').step6({ page, ctx, phone, tag, check, facts, whole, waitSaved, open, touchDrag, mouseDrag, center, ROOT, TOTAL, OUT });

  // ── NOTHING ELSE WENT WRONG ────────────────────────────────────────────────────────────
  const errors = log.errors.filter((e) => !envNoise(e));
  check(`[${tag}] zero CSP reports`, log.csp.length === 0, log.csp.join(' | ').slice(0, 400));
