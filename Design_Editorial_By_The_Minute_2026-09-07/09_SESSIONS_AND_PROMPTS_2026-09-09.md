...
| **S5** | 🔶 **HALF LANDED.** Step 1.1 (the route move to `/dashboard/[eventId]/story`) is **MERGED** — PR [#5337](https://github.com/iscasasola/setnayan-platform/pull/5337). Step 1.2 (the desk itself) is **OPEN** — PR [#5338](https://github.com/iscasasola/setnayan-platform/pull/5338). Do not start S6 or S7 until 1.2 lands. |
| **S9** | 🔶 **BUILT, PR [#5342](https://github.com/iscasasola/setnayan-platform/pull/5342) — OPEN with auto-merge armed as of 2026-09-09.** ⚠ **VERIFY WITH `gh pr view 5342 --json state,mergedAt` BEFORE TRUSTING THIS LINE** — a checkmark in a register is not evidence, and this corpus has been wrong about a PR's state five times. No migration. The cover REPLACES the masthead and the lead (it does not sit on top of them); `editionVolume`/`toRoman` MOVED into `lib/story-spine.ts`, `EditionLine`/`editionCenter`/`nameplate` deleted. ✅ **The inherited veto defect below is fixed** — and the subtraction is exact, not blanket: a vetoed capture WITH a baked blurred stand-in is on the page, so it keeps its height. ⏭ **NOT built, named:** the per-bin presign route (08 step 0.2's last clause) — the minute sheet names the minute, its count, the place and the nearest written moment, but shows no photographs. |
