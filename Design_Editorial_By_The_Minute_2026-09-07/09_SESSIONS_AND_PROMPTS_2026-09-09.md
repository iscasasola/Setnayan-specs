...
| **S8** | Publish — three states, the consent tick, the edition number stamped once | **Opus 5** | **high** | S5 · S6 | ✅ built |
...
| **S14** ✅ | A guest changes their mind **after** publish and it actually comes down | **Opus 5** | **high** | S8 | ✅ Q6 · **BUILT #5371** |
...
| **S8** | 🔶 **BUILT 2026-09-09 — PR pending; verify with `gh pr view <n> --json state,mergedAt` before trusting this row.** The publish ladder, the consent tick, the edition stamped once, **and both of `03`'s open defects it was told to carry: §2.4 (the number was recomputed at every render) and §2.8 (the room was read live).** 🔑 **RULE 0 PAID FIRST: the three states ALREADY SHIPPED** — `lib/who-can-see-your-story.ts` with `storyAudienceAdmits` as the one gate, and the DB CHECK already permits `draft·event·published`. Nothing about the audience model was rebuilt; what was missing was the SCREEN the design specifies for it, and the gate. ⚠ **One departure from the prototype, recorded in `02` §8:** the prototype selects a state then presses a separate *"Publish the story"*; here the RUNG is the press, which is the shipped editor's own decision and the stronger reason. 🪤 **AND THREE OF ITS OWN GUARDS WERE DECORATION, EACH FOUND BY MEASURING RATHER THAN BY READING:** `deskClear = true` hardcoded took the derivation 1 → 0 with the guard still 8/8 green (*it watched the wiring, not the value*); `if (false) return frozen;` took the freeze 1 → 0 with 21/21 green (*it matched the statement, not its guard clause*); and replacing the consent sentence in the JSX took its rendering 2 → 1 — **the survivor was the IMPORT** — with the loss checklist still reporting 0 missing (*an import is not a rendering*). All three fail now. ⏭ **Q5 stays open and the filter stays on weddings**; its `WEDDING_ONLY_BY_DESIGN` entry MOVED with the count, from `data.ts` to `lib/story-edition.ts`. ⇒ **S7 IS UNBLOCKED.** |
| **S8** | ✅ **MERGED + SERVED 2026-09-09** — PR [#5364](https://github.com/iscasasola/setnayan-platform/pull/5364), verified an ancestor of the commit production self-reports. Three publish states, the consent tick, and an edition number stamped once. ⇒ **S7 AND S14 ARE UNBLOCKED.** |
| **S11** | 🔶 **BUILT, PR [#5365](https://github.com/iscasasola/setnayan-platform/pull/5365) OPEN**, auto-merge armed, checks running. ⚠ It shifted six PRODUCTION rows to test the dial and **restored them** (verified: 0 of 14 captures now fall on the event day). Its restore script was never in the PR — **a test that edits live rows must commit its own undo.** |
| **S14** | ✅ **BUILT 2026-09-09 — PR [#5371](https://github.com/iscasasola/setnayan-platform/pull/5371) OPEN, auto-merge armed.** ⚠ Verify with `gh pr view 5371 --json state,mergedAt` before trusting this row. Nine consent writes now reach ONE list; the share card is busted by **moving its URL** (`?v={story_version_at}`) because `revalidatePath` cannot touch a `Cache-Control` header — a call there would have looked like a fix and done nothing. The fourth rung ships, offered only to a story that has actually been published (asked of the stamped `edition_no`, the one fact the database will not move). The keepsake carries the moment it was true, in the celebration's own zone. 🔑 **DERIVING THE POPULATION INSTEAD OF LISTING IT FOUND FOUR WRITERS THE DESIGN HAD NOT COUNTED** — the account-level RA 10173 opt-out (it reached one host screen), the RSVP selfie and the day-of face enrolment (both **lift** a veto, which publishes as urgently as taking one down), and **soft-deleting a guest, which un-vetoes every capture that tagged them — a widening hidden inside a delete.** ⚖ That last rule is OWNER territory and was not changed; only the caches now find out. 🪤 **AND THE GUARD ACCUSED CORRECT CODE TWICE BEFORE IT EVER MISSED ANYTHING** — asking its three questions of a whole function body convicted `markResponse`; asking them of two *different* builder chains convicted `submitRsvp`; and the banned phrase *"comes down … from a printed"* matched the honest sentence it exists to protect, because the window ran straight past the word **never**. *A cheaper proxy does not merely miss — it convicts, and the tempting way out is editing correct code until the guard is happy.* 🪤 `npx tsx --test` on the desk's `[eventId]` path ran **0 tests and exited 0**; only the glob form runs them. |
| ⏭ **LEFT** | **S7 · S12 · S13 · S15.** Ten of fifteen are done or in checks. ⛔ **S7 is the desk lane** (S8 just cleared it). ⛔ **S12 and S13 are the page lane — neither may run while S11 is open.** |
