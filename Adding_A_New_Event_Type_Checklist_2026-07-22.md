...
_History: `gala_night` (registered but missed by the reach study — grep vs. vocab), then `date`/`hangout` (registered 2026-07-22) exercised this net; the checklist-label miss is exactly item §2 row 1. Then `funeral` (2026-08-24, W4-WORDS, PR #4793) walked the whole checklist and found the net has grown beyond it — SIX coverage guards now fire on a new type (this list's three, plus the Papic offer, the recur-cadence map, the sample-story bill, and **host role sets**). ⚠ The role-set guard lives in `test:db` only, so it reaches you in CI unless you run the whole db suite locally — run `pnpm test:db`, not just your own new db files. Also new since this list was written: the terminology JSONB accepts tone keys (`register`/`occasion_noun`), and the admin profile upsert now merges over the stored blob rather than rebuilding it._
