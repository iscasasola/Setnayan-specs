...
| **Customer · in-app Services tab** | `0021_couple_dashboard_fully_purchased.md` · prototypes | The Services tab surfaces the 5 services under a **"Digital Services"** group, each with the **✦ Setnayan** badge + the **canonical name** (fixes the Paparazzi / LED Background / Invitation Bundle drift). |
...
- `service_catalog` seed (`0034`) — **Pakanta · Pro Website · Live Venue Photo Wall have NO SKU rows yet**; Animated Monogram's `category` should move `monogram → digital_services`; Pailaw needs an explicit SKU. Admin can't price what isn't seeded.
