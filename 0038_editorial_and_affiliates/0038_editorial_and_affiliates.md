...
> - **FIRST SLICE SHIPPED 2026-06-13 (PR [#1344](https://github.com/iscasasola/setnayan-platform/pull/1344)).** The editorial surface is now built: `/blog` index + `/blog/[slug]` SSG articles (in-code typed constants in `lib/blog.ts`, mirroring the shipped `/help/[slug]` pattern — no DB, no CMS, no markdown-renderer dep, `dynamicParams=false` → real 404s), 9 PH-anchored long-tail seed articles (4 in PR #1344 + 5 in #1346), `BlogPosting`/`Blog`/`ItemList`/`BreadcrumbList` JSON-LD, `sitemap-blog.xml` (registered in the sitemap index) + footer "Journal" link + `llms.txt` entry. **STILL NOT BUILT (deferred to later slices):** `/recommendations`, `/recommendations/[category]`, admin `/editorial`, and the `editorial_articles` / `recommendation_pages` / `affiliate_links` / `affiliate_conversions` / `sponsored_slot_bookings` tables — the affiliate/recommendations/sponsored monetization layer needs DB tables + monetization decisions. The blog content is git-tracked TS constants (NOT the `content/editorial/*.md` files this spec describes — the as-built help center uses in-code constants, and the blog mirrors it).
