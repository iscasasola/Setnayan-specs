...
> - **The Save-the-Date Video add-on SHIPPED** at `/dashboard/[eventId]/add-ons/save-the-date` (12-template gallery → inline checkout → `service_key='save_the_date_video'`), but it is **NOT one of the live-site public SKUs** — the ground-truth catalog (5 Live / 5 In-build / 9 Coming-soon + 2 bundles + Setnayan AI) does not list it. Treat it as a shipped in-app add-on, not a publicly-advertised product.
