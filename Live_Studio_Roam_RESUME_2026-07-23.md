...
## TL;DR
**Live Studio = 2 variants** (owner 2026-07-23): **Cast ₱2,500/day** (existing directed single-feed; couple's own OBS→own YouTube; ~₱0 to us) + **Roam ₱3,500/day** (NEW: guests pick which camera / wander the venue — multi-cam, multi-venue, on the event page, directed feed as default). Roam streams on a **Setnayan-owned YouTube channel pool** _(⚠ **SUPERSEDED 2026-07-25 — owner-locked: broadcasts go on the COUPLE'S OWN channel.** A channel per event is not buildable (Google rate-limits creation; each needs phone verification + a 24-hour wait before its first live stream), and one-channel-per-event was the only thing making a pool safe — concentrated weddings mean one strike takes them all down. `live_studio_roam_channel_pool` exists but holds 0 rows and its admin screen was never built; leave it dormant. A 5–10 channel pool may return ONLY as a fallback for couples with no channel. See `DECISION_LOG.md` 2026-07-25.)_ (₱0 streaming — cameras push direct to YouTube; YouTube absorbs viewers). Base Roam = **cameras BYO** (phones join via QR claim, ~₱0 COGS like Cast); a **Setnayan camera kit is a SEPARATE add-on** (not included at ₱3,500 — owner to confirm). Naming: code namespace = `live_studio_roam_*`; legacy Cast keeps `panood_*`.
