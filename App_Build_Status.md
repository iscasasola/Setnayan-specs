...
| 0016 Step-by-Step Plan Builder | renamed in spec to "Setnayan AI" (CLAUDE.md 2026-05-24 eighth row), engineering shipped 65-card wizard | 38 → 65 cards across PRs #519–#544 (one-PR-per-card waves); /today route + first BottomNav tab "Today" with Focus icon shipped via PR #520; 6-week Phase 0–5 plan locked 2026-05-23 row 6 collapsed into ~5-day intensive engineering sprint 2026-05-24 |
...
| 0021 Couple Dashboard | ✅ shipped → ✅ refreshed for Finder-column UX (2026-05-22 PR #367) → ✅ Setnayan AI extracted to /today (PR #520) | Per-event localStorage isolation fix (PR #517) + cap-compare-at-2 (PR #515) closed customer-side bug-hunt findings |
...
| 0034 Payments & Cart | ✅ schema shipped — but **Setnayan Pay fee is DORMANT, not charged**: all 6 `setnayan_pay_methods` rows are `is_active=FALSE` (verified prod DB 2026-06-29). Stored `setnayan_pay_pct=0.05` never applies; effective convenience fee = **0%** per the 2026-06-07 reset. | Migration `20260608000000_iteration_0023_setnayan_pay_rate_alignment.sql` shipped per PR #432 (rate config only; rail switched off) |
...
1. **Setnayan AI SKU model (2026-05-24 eighth row)** — supersedes 2026-05-17 Concierge `concierge_complete` ₱2,499 single-tier + 2026-05-18 row 1 Concierge AI Brain paid-tier-gating + 2026-05-22 row 3 CONCIERGE_ENABLED killswitch. New SKUs `todays_focus` ₱9,999 one-time per-event + `todays_focus_extension` ₱4,999 discounted re-up, 24mo cumulative-active pause-aware runtime, per-host `users.show_todays_focus_wizard` toggle for DIY parity. **🟡 Decision LOCK shipped to spec corpus; engineering migration NOT yet shipped.** Pilot-friendly because CONCIERGE_ENABLED=off for pilot per 2026-05-22 row 3, so the wizard surface is hidden during pilot — pilot couples interact with the schema reality (concierge_complete ₱2,499 SKU still active) via Your Plan grid + marketplace search only. Customer-facing copy on `/pricing` + `/for-vendors` + `/waitlist` + `/privacy` still markets "Setnayan Concierge ₱2,499" — consistent with schema. Owner decision pending: rename to Setnayan AI before vs after pilot.
...
1. **Setnayan AI SKU migration** — schema additions (`events.todays_focus_*` columns + `users.show_todays_focus_wizard` + 2 service_catalog SKU rows) + activation/pause/resume logic + DIY toggle UI. Pilot-friendly to defer since CONCIERGE_ENABLED=off for pilot.
...
⚠️ /pricing "flat 5.0% + ₱100K worked example" — STALE: convenience fee RETIRED to 0% (2026-06-07 reset); verify live copy no longer shows it
✅ /privacy has 4 RA 10173 sections
⚠️ Setnayan Pay 5.0% bps + ₱50 min floor — DORMANT (all `setnayan_pay_methods` rows `is_active=FALSE`, verified 2026-06-29); effective fee 0%. BIR 0.5% withholding unaffected
...
🟡 Concierge → Setnayan AI SKU rename — defer to post-pilot decision
...
| 📐 **V1 architectural locks shipped to spec but engineering deferred** (2026-05-23 → 2026-05-28) | 7 | Setnayan AI SKU model · Lock/delete/overlap 5-rule architecture · 5-tier recommendation ladder · Package-inclusion auto-tagging · Per-card hard-floor scheduler · Vendor Pattern A V1.1 multi-photo tile upgrade · Caterer venue-recommended integration (V1 SCOPE FLAG pending owner re-confirm) |
| ⛔ Blocked | 0 | — |
...
| 0001 | Creating Guest List | ✅ Shipped (grown to ~15-surface guest lifecycle) | RSVP-received email + in-app notification (PR #20). **Spec-vs-shipped audit 2026-07-08** → `0001_creating_guest_list/0001_Guest_Spec_vs_Shipped_Audit_2026-07-08.md`: core 0001 faithful; feature now spans check-in / souvenir / tea-ceremony / mind-map / groups / claims / bulk-ops / multi-role / pax-lock / biometrics / person-spine (`guest_role` enum 18→~32). Result-doc drift (3 wrong facts) corrected. |
...
| 0011 | Live Studio (formerly Panood) | ⚙️ OAuth + cron + UI scaffolds shipped 2026-05-16; awaiting Google verified-app review | **SKU lock 2026-05-16** (see prior history below). _Renamed 2026-06-29: Panood → Live Studio; paid multicam tier repriced ₱4,999 → ₱3,499/day (internal SKU keys unchanged). See DECISION_LOG.md 2026-06-29._ YouTube OAuth code shipped 2026-05-16 via commit `20b21fc`: `lib/panood-youtube.ts` + 3 routes (`/api/oauth/youtube/start` + `/callback` + `/disconnect`) + `/api/cron/oauth-refresh` + UI scaffolds `/dashboard/[eventId]/add-ons/panood/{page,setup,broadcast,reviews}`. **Owner action:** YouTube verified-app submission Phase 2 still pending (#17a — privacy disclosure ✅ shipped PR #116, demo video pending owner). |
| 0012 | Papic | ⚙️ Schema + storage target + V1 SKU seed + Drive OAuth shipped 2026-05-19/20 | **⚠ 2026-06-28 correction (body below is a 2026-05 snapshot — current state in DECISION_LOG + [[project_setnayan_papic_completion_program]]):** since shipped — per-camera pricing · **offline capture queue (seat + guest, PRs #2353/#2358)** · **adaptive compression (#2359)** · **SDE retired fully (#2362 + #2370 + prod migration `20270316029217`)** · **Auto-Recap "brain" (selection #2367 + 30s FFmpeg builder #2363); server render staged for Oracle A1 Always-Free, owner-gated on box + Suno/CC0 music**. Render outputs = Stories (free) + Auto-Recap (≤30s); native iOS/Android still V1.5+. _Original snapshot:_ Migrations 55 (`events_papic_storage_target.sql`), 73 (`v1_sku_lock_papic_seat_packs.sql`), 75 (`iteration_0012_paparazzi_seats_photos.sql`) shipped via PRs #149 + #151. `lib/papic-drive.ts` + 3 OAuth routes (`/api/oauth/drive/start` + `/callback` + `/disconnect`) shipped. UI route `/dashboard/[eventId]/add-ons/papic/page.tsx` shipped as scaffold. **Architecture lock 2026-05-16** retained (see history below — 207-camera mesh, Drive transfer at T+30d, pooled credits, Auto-Recap). **Native iOS/Android still V1.5+** — web-side schema + Drive integration now landed. |
...
| Live Studio (formerly Panood) live stream | Provision Cloudflare Stream Live + YouTube Data API + master channel |
