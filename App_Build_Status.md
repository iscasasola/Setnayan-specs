...
| 🟡 V1 build pending (promoted from V1.5+ 2026-05-18) | 6 | 0005, 0009, 0011, 0012, 0017, 0018 |
| 📐 V1.1 spec drafted (2026-05-19) | 7 | 0043, 0044, 0045, 0046, 0047, **0038**, **0039** (last two added 2026-05-19 traffic-monetization scope expansion) |
...
| 0005 | LED Background | 🟡 V1 build pending (promoted from V1.5+ 2026-05-18) | Now visible as "Coming soon" in the add-ons grid; promotion is spec-side only — engineering capacity planning pending. |
...
| 0017 | Patiktok | 🟡 V1 build pending (promoted from V1.5+ 2026-05-18 · SKU lock 2026-05-16) | **SKU lock 2026-05-16:** old single-tier ₱2,499/booth/5hr + ₱999 additional-station + ₱499/hour retired. New dual-tier per-day model: Patiktok Setnayan TikTok ₱999/day (auto-posts to @SetnayanWeddings · Setnayan keeps ad-revenue) vs Patiktok Personal TikTok ₱1,999/day (couple BYO TikTok via OAuth · couple owns videos). 40-video soft cap per booth per day · ₱49/+10 overage SKU as in-event upsell · per-day multi-purchase for multi-day events. Sound selection / multi-performer / external-display mechanics preserved. Anti-abuse rule changed: 500-per-event hard cap → 40-per-day soft cap. Still showing as "Coming soon" in add-ons grid pending V1 build (promoted from V1.5+ on 2026-05-18). |
| 0018 | Supplies Marketplace | 🟡 V1 build pending (promoted from V1.5+ 2026-05-18) | Now visible as "Coming soon" in add-ons grid (PR #22); promotion is spec-side only — engineering capacity planning pending. |
...
| **0043** | **Wedding Type Picker** (V1.1 NEW spec) | 📐 Spec drafted 2026-05-19 | Two-axis picker (Ceremony × Venue) + conditional sub-types + secondary ceremony; schema for all 7 ceremony types + 7 venue settings + wedding_type_launch_status (admin-toggleable per-faith per-region visibility) + couple_wedding_type_notify_signups (email capture for inactive faiths). V1.1 ships 2 active (Catholic + Civil) + 4 Coming Soon (INC / Christian / Muslim / Cultural) with vendor compatibility tags from day 1 so non-Catholic vendor recruitment can pre-seed before visibility flip. |
...
| **0039** | **Display Ads** (V1.1 NEW spec · traffic monetization · activation-gated) | 📐 Spec drafted 2026-05-19 · ⛔ activation gated on owner sign-off | Google AdSense as V1.1 single network. Public, pre-purchase surfaces only — marketing site + help articles + 0038 editorial + marketplace discovery + vendor landing pages. **Site-wide RA 10173 cookie-consent banner** (new system surface; 3 categories essential/analytics/advertising; 12-month persistence; first-party `sn_consent_v1` cookie + `users.consent_state` JSONB). Hard guardrails: max 1 unit/page · AdSense topic-exclusion of wedding/event categories on vendor profiles + editorial + recommendations + marketplace · Auto Ads OFF · no interstitials. Vendor opt-out toggle on `/vendors/[slug]` (Boosted Ads / Sponsored Boost vendors default OFF). Two-admin activation kill-switch in 0023 § 5.1. Excludes logged-in dashboards · guest landing 0002 incl. Phase 4 · day-of 0031 · sponsored articles · sponsored newsletter slots · checkout · contact form · cookie-preferences · error pages · vendor verification flows. New tables: `cookie_consent_events` + `adsense_activation_log` + `adsense_daily_revenue`; ALTERs on `users` + `vendors`. CSP middleware update. **Yield ~₱5-20K/mo at 100K pageviews vs Boosted Ads ~₱780K/yr per 20km vendor — two orders of magnitude gap surfaced; owner picked "all public pages" with brand trade-off acknowledged.** |
